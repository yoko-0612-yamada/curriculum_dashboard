...
# 3) 💻ツール内で作った予定は、更新・キャンセル・誤登録訂正を従来どおり利用可能。
# 4) 将来ツール→座席予約スケジュールの逆同期が完成した時点で外部受付予定の更新を解禁する想定。
# 5) d389までの訂正履歴・取消線・出席/未完了除外・受付経路絵文字・8種類request_typeは維持。

# d389: 予定操作の意味を「キャンセル／誤登録を訂正／変更・振替」に整理
# 1) 「完全削除」という通常操作名をやめ、「誤登録を訂正」に変更。
# 2) キャンセル＝実際に存在した予定を取りやめ。グレー＋取消線で履歴を残す。
# 3) 誤登録を訂正＝入力ミスの予定を現在のカレンダーから消すが、訂正履歴は別ログへ残す。
# 4) 変更・振替＝既存の処理を維持。
# 5) 外部受付由来の予定は、連携履歴を壊さないためツール側で誤登録訂正しない。

# d388: 「今日の未完了」からキャンセル済み予定を除外
# 1) d387の「今日来る人／出席記録」のキャンセル除外を維持。
# 2) 未完了タスク生成用の plan_day でも、取消線表示用のキャンセル行を除外。
# 3) 月カレンダーのグレー＋取消線表示、受付経路絵文字はそのまま維持。
# 4) 8種類request_type、CSV取込、再追加判定などは変更しない。

# d387: 出席記録の「今日来る人」からキャンセル済み予定を除外
# 1) 今日の予定・出席記録では、取消線表示用のキャンセル行を現在有効な予定として扱わない。
# 2) 口頭受付・LINE Bot・ツール内操作のどこからキャンセルされても、出席対象から除外。
# 3) 月カレンダーのグレー＋取消線表示はそのまま維持。
# 4) 8種類request_type、受付経路絵文字、再追加判定などは変更しない。

# d386: キャンセル表示統一＋受付経路アイコン
# 1) 座席予約連携キャンセルも通常キャンセルと同じ override_status='キャンセル' を使い、
#    名前・予定ともグレー＋取消線の既存表示へ統一。
# 2) 追加・キャンセル・変更の受付経路を絵文字で表示。
#    💻=ツール内操作 / 😃=口頭受付 / 📱=LINE Bot
# 3) 座席予約連携CSVのreception_sourceをschedule_overridesのnoteへ保持。
# 4) 既存の座席予約連携データは、受付経路情報が無い場合は現在の運用に合わせ😃として表示。
# 5) CSV取込・request_type・再追加判定などの処理ロジックは変更しない。

# d385: 取消線表示だけを限定修正（回帰影響を最小化）
# 1) 座席予約連携キャンセル行をカレンダーへ戻す際に、表示に必要な列を明示的に保持。
# 2) date / weekday / slot / session_type / display_name / action / status /
#    reason / cancelled / is_cancelled を欠落させない。
# 3) 8種類request_type、CSV取込、反映済み判定、再追加重複判定、
#    通常追加・振替ロジックには手を入れない。
# 4) 今回のテストは「キャンセル→取消線表示」だけで確認できるようにする。

# d384: 取消線表示用の予定を「追加先の重複」として扱わないよう修正
# 1) カレンダーに取消線で残している行は、追加先重複チェックから除外。
# 2) 「追加→キャンセル→同じ日時へ再追加」を正常に反映可能にする。
# 3) 現在有効な予定だけを追加先重複として判定。
# 4) d383の最新状態表示、d381の取消線表示、8種類request_type連携を維持。

# d383: 座席予約連携の同一日時「キャンセル→再追加」で最新受付を優先
# 1) schedule_overridesに過去のキャンセル行が残っていても、
#    同じ生徒・日付・コマの座席予約連携では最後に登録された受付を有効とする。
# 2) 再追加が最後なら旧キャンセルを表示判定から除外。
# 3) キャンセルが最後なら取消線表示を維持。
# 4) 既にd381/d382で取り込み済みのデータも、CSVを再取込せず表示を復旧できる。
# 5) 手動キャンセル等、座席予約連携以外の例外は変更しない。

# d382: 座席予約連携でキャンセル後、同じ日時へ再追加した時に予定を復活できるよう修正
# 1) 新しい追加/振替先を反映する直前に、同じ生徒・日付・コマの
#    「座席予約連携由来のキャンセル」だけを解除。
# 2) 手動キャンセルなど、座席予約連携以外のキャンセルは勝手に解除しない。
# 3) d381の取消線表示、d380の8種類request_type連携・二重反映防止は維持。

# d381: 座席予約連携のキャンセルを月カレンダーで取消線表示に修正
# 1) lesson_cancel / selfstudy_cancel で元予定を消さず、取消済み行として表示。
# 2) 座席予約連携由来のキャンセル行も、月カレンダーに元予定情報を残して見える化。
# 3) d380の8種類request_type連携・二重反映防止は維持。

# d380: 座席予約スケジュールv3.5の8種類受付CSV取り込み対応
# 1) request_typeで8種類の受付を分岐。
# 2) 授業/自習の元予定種別を検証し、追加先重複も確認。
# 3) 振替は元取消＋先追加、追加は先追加のみ、キャンセルは元取消のみ。
# 4) 授業↔自習の同日同コマ変更は時間変更として反映し、取消に潰されないようにする。
# 5) reservation_idによる二重反映防止は従来どおり維持。
...
# 3) 生徒管理も未保存時の案内を強化し、長い画面の上部にも保存ボタンを追加。
# 4) 既存の下部保存ボタンは残し、上部/下部どちらからでも同じ一括保存処理を実行。

# d369: 退会済み表示・授業/自習視認性・月予定の種別/回数区分整合性を改善
# 1) 閲覧サイドバーの生徒候補は在籍中を基本表示。退会済みは「含める」ON時だけ表示。
# 2) 「今日の未完了」「次に見る候補」で 📘授業 / 📝自習 を明示。
# 3) 月スケジュール「新しい予定」で、種別に応じて回数区分候補を連動。自習は「自習（回数外）」に固定。
# 4) 保存時にも種別と回数区分を再検証し、矛盾した組み合わせを保存しない。

# d368: 出欠回数と当日スケジュール回数の整合性チェック
# 1) 未登録の出欠回数は、当日の有効スケジュールから自動で初期値を設定。
# 2) スケジュール回数と異なる回数へ変更した場合は警告し、明示確認がないと保存できない。
# 3) 保存済み出欠とスケジュール回数が不一致の生徒を、出欠画面上部で一覧警告。
# 4) 例外運用を禁止せず、実績が予定と違う場合は確認のうえ保存可能。

# d367:
# Bot用匿名スケジュールCSV出力を追加。
# 1) 管理（入力）に「🤖 Bot匿名CSV」を追加。
# 2) 生徒名をサーバーへ出さず、ローカル専用の匿名ID対応表を保持。
# 3) 月スケジュール・固定スケジュール・schedule_overrides.csv を共通予定ロジックで統合し、
#    指定期間の bot_schedule.csv を生成。
# 4) bot_schedule.csv には実名・student_id・学年・メモ等の個人情報を含めない。
# 5) CSV生成時に個人情報列が混入していないか安全チェックを実施。
#
# d366:
# 月スケジュール回数確認の一本化。
# 1) 上部の独立「📊 回数確認」モードを削除。
# 2) カレンダー内の「予定回数チェック」を「📊 回数確認（予定ベース）」へ統一。
# 3) schedule_overrides.csv のキャンセルを反映し、キャンセル済み予定は回数から除外。
# 4) 回数に含む／含めない対象を画面上に明記。
#
# d365:
# 月回数チェック統合版。
# 1) 閲覧側の「📅 今月回数一覧」を削除。
# 2) 月回数チェックは「管理（入力）→ 月スケジュール → 📊 回数確認」に一本化。
# 3) 回数確認の上部に、集計対象／対象外／予定回数であることを明記。
#
# d364:
# UI統合版。
# 1) 「📘 課題を登録・編集する（管理へ移動）」ボタンを削除（説明文は維持）。
# 2) 独立していた「座席」ページを廃止。
# 3) 管理（入力）→ 月スケジュール内に「🪑 今日の座席」を追加。
# 4) 月カレンダー／今日の座席／回数確認／生成を同じ画面内で切替可能。
# d363:
# 振替インボックスに「自習追加」を追加。
# 元予定は変更せず、指定日・コマへ session_type=自習 の追加予定を反映。
# 同一生徒・日付・コマの重複チェックを維持。
# カレンダーでは振替追加と区別して「自習追加」と表示。
# d362:
# 振替インボックスに「振替（元予定取消＋振替先追加）」を追加。
# 1件の受付に source_date/source_slot と requested_date/slot を保持。
# 承認時に元予定をキャンセルし、振替先を追加。元座席も解除。
# 既存の「振替追加」「欠席」も継続利用可能。
# d361:
# 振替インボックスの未承認受付に「削除（却下）」を追加。
# 削除時は完全削除せず status=rejected として履歴を保持。
# 未承認一覧からは消え、下部の「却下済み」履歴で確認可能。
# d360:
# 振替インボックスを拡張。
# 1) コマ表示を「コマ＋時間」に統一。
# 2) インボックス承認の追加予定は「当日追加」ではなく「振替追加」と表示。
# 3) 受付内容に「欠席」を追加。
# 4) 欠席承認時は schedule_overrides.csv にキャンセルを登録し、座席も解除。
# 旧transfer_inbox.csvはrequest_type空欄を「振替追加」として後方互換。
# d359:
# 振替インボックスMVPを追加。
# 1件登録 → pending一覧 → 承認 → schedule_overrides.csvへ「追加」で反映。
# 承認後はtransfer_inbox.csvをapprovedへ更新し、承認履歴を保持。
# d358:
# 管理（入力）に「🎯 検定課題管理」を新設。
# kentei_tasks.csv の検定課題を追加・編集・削除・順番変更できる。
# 閲覧側の検定課題は進捗登録、管理側は課題マスタ管理として役割を分離。
# d357:
# 検定課題に「学習順（order）」編集UIを追加。
# 現在の級の課題ごとに順番番号を変更して kentei_tasks.csv へ保存できる。
# 課題一覧・今日やる候補は、完了状態ではなく order 順を基準に表示する。
# d356:
# コース単位スキップを「現在のコース」候補から除外。
# course_done と course_skip の両方を終了扱いにし、
# スキップ済みコースが推奨・フォールバックで再表示されないよう修正。
# d355:
# カリキュラム単位の「スキップ」を追加。
# progress_log.csv に status=skip / note=course_skip として履歴を保存。
# 完了とスキップは排他的にし、編集ロック解除時はスキップ取消も可能。
# 未来予測ではコース完了／コーススキップ済みを現在コース候補から除外。
# d334:
# 未来予測を全員一覧で確認できるように拡張。
# 一覧で現在コース・進捗・残り・修了予測・Scratch現在級・1級予測を表示。
# その下に生徒ごとの詳細予測を残し、一覧→詳細の流れに変更。
# d333:
# 未来予測（試算）を閲覧 → たまに見る：確認・一覧 に追加。
# 1) 現在のカリキュラム修了まで：残課題数と直近90日の完了ペースから試算。
# 2) Scratch検定1級まで：合格履歴が2回以上ある場合、級間の実績ペースから試算。
# 予測は評価ではなく目標設定用。データ不足時は無理に数値を出さない。
# d332:
...
# [KEEP 2026-04-23] このファイル内で参照あり。現時点では使用中として維持。
def append_schedule_correction_log(items) -> None:
    if not items:
        return
    ...
    for item in items:
        rows.append({
            "corrected_at": now,
            "operation": "誤登録を訂正",
            "source": str(item.get("source", "💻 ツール")).strip(),
    ...
    new_df = pd.DataFrame(rows, columns=SCHEDULE_CORRECTION_LOG_COLS)

    if SCHEDULE_CORRECTION_LOG_CSV.exists():
        try:
            old_df = pd.read_csv(
                SCHEDULE_CORRECTION_LOG_CSV,
                dtype=str,
                encoding="utf-8-sig",
            ).fillna("")
        except Exception:
            old_df = pd.DataFrame(columns=SCHEDULE_CORRECTION_LOG_COLS)
    else:
        old_df = pd.DataFrame(columns=SCHEDULE_CORRECTION_LOG_COLS)

    for c in SCHEDULE_CORRECTION_LOG_COLS:
        if c not in old_df.columns:
            old_df[c] = ""

    merged = pd.concat(
        [old_df[SCHEDULE_CORRECTION_LOG_COLS], new_df],
        ignore_index=True,
    )
    write_csv_atomic(merged, SCHEDULE_CORRECTION_LOG_CSV)
...
SCHEDULE_CORRECTION_LOG_CSV = DATA_DIR / "schedule_correction_log.csv"
SCHEDULE_CORRECTION_LOG_COLS = [
    "corrected_at", "operation", "source", "student_id",
    "date", "slot", "session_type", "reason", "note",
]
...
# d359: 振替受付の一時保管箱
TRANSFER_INBOX_CSV = DATA_DIR / "transfer_inbox.csv"
...
SEAT_TRANSFER_IMPORT_REQUIRED_COLS = [
    "reservation_id", "bot_student_id", "request_type",
    "source_date", "source_slot", "requested_date", "requested_slot",
    "source_session_type", "requested_session_type",
    "reception_source", "status",
]
SEAT_TRANSFER_IMPORT_LOG_COLS = [
    "reservation_id", "bot_student_id", "student_id", "request_type",
    "source_date", "source_slot", "requested_date", "requested_slot",
    "source_session_type", "requested_session_type",
...
SEAT_REQUEST_RULES = {
    "lesson_transfer": {"needs_source": True, "needs_dest": True, "source_type": "授業", "dest_type": "授業"},
    "lesson_add": {"needs_source": False, "needs_dest": True, "source_type": "", "dest_type": "授業"},
    "lesson_cancel": {"needs_source": True, "needs_dest": False, "source_type": "授業", "dest_type": ""},
    "selfstudy_transfer": {"needs_source": True, "needs_dest": True, "source_type": "自習", "dest_type": "自習"},
    "selfstudy_add": {"needs_source": False, "needs_dest": True, "source_type": "", "dest_type": "自習"},
    "selfstudy_cancel": {"needs_source": True, "needs_dest": False, "source_type": "自習", "dest_type": ""},
    "lesson_to_selfstudy": {"needs_source": True, "needs_dest": True, "source_type": "授業", "dest_type": "自習"},
    "selfstudy_to_lesson": {"needs_source": True, "needs_dest": True, "source_type": "自習", "dest_type": "授業"},
}

SEAT_REQUEST_LABELS = {
    "lesson_transfer": "授業→授業振替",
    "lesson_add": "授業追加",
    "lesson_cancel": "授業キャンセル",
    "selfstudy_transfer": "自習→自習振替",
    "selfstudy_add": "自習追加",
    "selfstudy_cancel": "自習キャンセル",
    "lesson_to_selfstudy": "授業→自習",
    "selfstudy_to_lesson": "自習→授業",
}
BOT_STUDENT_MAP_COLS = ["student_id", "bot_student_id"]
BOT_SCHEDULE_COLS = [
    "bot_student_id", "date", "weekday", "slot",
    "start", "end", "session_type",
]

def load_bot_student_map_private() -> pd.DataFrame:
    """d367: ローカル専用の student_id ↔ bot_student_id 対応表を読む。"""
    if not BOT_STUDENT_MAP_PRIVATE_CSV.exists():
        return pd.DataFrame(columns=BOT_STUDENT_MAP_COLS)
    df = safe_read_csv(
        BOT_STUDENT_MAP_PRIVATE_CSV,
        BOT_STUDENT_MAP_COLS,
        stop_on_missing=False,
        show_message=False,
    )
    for c in BOT_STUDENT_MAP_COLS:
        if c not in df.columns:
            df[c] = ""
        df[c] = df[c].fillna("").astype(str).str.strip()
...
def _seat_origin_source_label(reception_source: str) -> str:
    s = str(reception_source or "").strip().lower()
    if "line" in s:
        return "📱 LINE Bot"
    if "口頭" in s:
        return "😃 口頭受付"
...
def _find_latest_linked_override(
    schedule_overrides_df: pd.DataFrame,
    *,
    reservation_id: str,
    student_id: str,
) -> dict:
    """reservation_idをnoteに持つ最新の例外行を探す。"""
    if schedule_overrides_df is None or schedule_overrides_df.empty:
        return {}
    ...
    hit = ov[
        ov["student_id"].eq(sid)
        & ov["note"].str.contains(rid, regex=False, na=False)
    ].copy()
    if hit.empty:
        return {}

    # schedule_overridesは行順を履歴順として扱う。
    r = hit.iloc[-1]
    return {
...
def build_seat_current_state_sync_df(
    *,
    ...
    """d392: 受付履歴ごとの「現在状態」をカリキュラム側から判定する。

    戻り値:
      export_df: v3.7へ渡す匿名CSV
      preview_df: 教室PC上だけで見る確認用（生徒名を含む）
    """
    log = import_log_df.copy() if isinstance(import_log_df, pd.DataFrame) else pd.DataFrame()
    if log.empty:
        return (
            pd.DataFrame(columns=SEAT_CURRENT_STATE_SYNC_COLS),
            pd.DataFrame(),
        )
    ...
    for c in SEAT_TRANSFER_IMPORT_LOG_COLS:
        if c not in log.columns:
            log[c] = ""
        log[c] = log[c].fillna("").astype(str).str.strip()
    ...
    def get_plan(d_str: str) -> pd.DataFrame:
        d_str = str(d_str).strip()
        if not d_str:
            return pd.DataFrame()
        if d_str not in plan_cache:
            plan_cache[d_str] = build_daily_plan_for_date(
                d_str,
                students_df,
                student_schedule_df,
                monthly_schedule_df,
    ...
    for _, r in log.iterrows():
        ...
        req = str(r.get("request_type", "")).strip() or "lesson_transfer"
        reception_source = str(r.get("reception_source", "")).strip()
        orig_date = str(r.get("requested_date", "")).strip()
        orig_slot = normalize_slot(r.get("requested_slot", ""))
        orig_type = str(r.get("requested_session_type", "")).strip()
        if not orig_type:
            orig_type = SEAT_REQUEST_RULES.get(req, {}).get("dest_type", "")

        current_date = orig_date
        current_slot = orig_slot
        current_type = orig_type
        current_status = "active"
        current_source = _seat_origin_source_label(reception_source)

        # 受付自体がキャンセル受付なら、その受付の現在状態はキャンセル済み。
        if req in ["lesson_cancel", "selfstudy_cancel"]:
            current_date = ""
            current_slot = ""
            current_type = ""
            ...
            linked = _find_latest_linked_override(
                schedule_overrides_df,
                reservation_id=rid,
                student_id=sid,
            )

            # 過去版で外部受付予定をツール側変更していた場合にも追従。
            if linked and linked.get("action") in ["追加", "時間変更"]:
                if linked.get("date"):
                    current_date = linked["date"]
                if linked.get("slot"):
                    current_slot = linked["slot"]
                if linked.get("session_type"):
                    current_type = linked["session_type"]

            # 現在の実予定に存在するかで最終判定する。
            plan = get_plan(current_date)
            active_hit = pd.DataFrame()
            if plan is not None and not plan.empty:
                tmp = plan.copy()
                for c in ["student_id", "slot", "session_type", "action", "status", "override_status", "reason"]:
                    if c not in tmp.columns:
                        tmp[c] = ""
                    tmp[c] = tmp[c].fillna("").astype(str).str.strip()
                ...
                active_hit = tmp[
                    tmp["student_id"].eq(sid)
                    & tmp["slot"].map(normalize_slot).eq(normalize_slot(current_slot))
                ].copy()

                if not active_hit.empty:
                    cancel_mask = pd.Series(False, index=active_hit.index)
                    for c in ["action", "status", "override_status", "reason"]:
                        vals = active_hit[c].fillna("").astype(str).str.strip()
                        cancel_mask = cancel_mask | vals.isin(
                            ["キャンセル", "取消", "取消済み", "キャンセル済み", "削除"]
                        )
                    for c in ["cancelled", "is_cancelled"]:
                        if c in active_hit.columns:
                            cancel_mask = cancel_mask | active_hit[c].map(
                                lambda v: (
                                    v is True
                                    or str(v).strip().lower() in ["true", "1", "yes"]
                                )
                            ).fillna(False)
                    active_hit = active_hit.loc[~cancel_mask].copy()

            if active_hit.empty:
                # 予定が無く、同位置にキャンセルがあるなら明確にcancelled。
                current_status = "cancelled"
                current_date = ""
                current_slot = ""
                current_type = ""
                current_source = "💻 カリキュラムツール"
            else:
                actual_type = str(active_hit.iloc[0].get("session_type", "")).strip()
                if actual_type:
                    current_type = actual_type

                changed = (
                    current_date != orig_date
                    or normalize_slot(current_slot) != normalize_slot(orig_slot)
                    or (orig_type and current_type != orig_type)
                )
                current_status = "changed" if changed else "active"
                if changed:
                    current_source = "💻 カリキュラムツール"
        ...
        preview_rows.append({
            "予約ID": rid,
            "生徒": name_map.get(sid, sid),
            "受付内容": SEAT_REQUEST_LABELS.get(req, req),
            "受付時": (
                f"{orig_date} / {orig_slot}コマ / {orig_type}"
                if orig_date or orig_slot or orig_type
                else "キャンセル受付"
            ),
            "現在": (
                f"{current_date} / {current_slot}コマ / {current_type}"
                if current_status in ["active", "changed"]
                else "予定なし"
            ),
            "現在状態": {
                "active": "🟢 現在有効",
            ...
            "最終変更元": current_source,
        })

    export_df = pd.DataFrame(export_rows, columns=SEAT_CURRENT_STATE_SYNC_COLS)
    preview_df = pd.DataFrame(preview_rows)
    return export_df, preview_df
def _normalize_transfer_import_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # v3.1以前の旧CSVも授業振替として後方互換。
    out.loc[out["request_type"].eq(""), "request_type"] = "lesson_transfer"
    out.loc[out["source_session_type"].eq(""), "source_session_type"] = (
        out["request_type"].map(
            lambda x: SEAT_REQUEST_RULES.get(x, {}).get("source_type", "")
        )
    )
    out.loc[out["requested_session_type"].eq(""), "requested_session_type"] = (
        out["request_type"].map(
            lambda x: SEAT_REQUEST_RULES.get(x, {}).get("dest_type", "")
        )
    )
...
def _plan_hit(plan_df: pd.DataFrame, sid: str, slot: str) -> pd.DataFrame:
    if plan_df is None or plan_df.empty:
        return pd.DataFrame()
    tmp = plan_df.copy()
    for c in ["student_id", "slot", "session_type"]:
        if c not in tmp.columns:
            tmp[c] = ""
        tmp[c] = tmp[c].fillna("").astype(str).str.strip()
    return tmp[
        tmp["student_id"].eq(str(sid).strip())
        & tmp["slot"].map(normalize_slot).eq(normalize_slot(slot))
    ].copy()
...
def _plan_hit_active(plan_df: pd.DataFrame, sid: str, slot: str) -> pd.DataFrame:
    ...
    for col in ["action", "status", "reason"]:
        if col in active.columns:
            ...
            cancel_mask = cancel_mask | vals.isin(
                ["キャンセル", "取消", "取消済み", "キャンセル済み"]
            )

    for col in ["cancelled", "is_cancelled"]:
        if col in active.columns:
            vals = active[col]
            bool_mask = vals.map(
                lambda v: (
...
def validate_seat_transfer_import_rows(
    ...
    *,
    ...
    """d380: 8種類の受付をrequest_typeごとに検証する。"""
    ...
    for _, r in work.iterrows():
        ...
        req = str(r.get("request_type", "")).strip()
        rule = SEAT_REQUEST_RULES.get(req)
        src_date = str(r.get("source_date", "")).strip()
        src_slot = normalize_slot(r.get("source_slot", ""))
        dst_date = str(r.get("requested_date", "")).strip()
        dst_slot = normalize_slot(r.get("requested_slot", ""))
        src_type = str(r.get("source_session_type", "")).strip()
        dst_type = str(r.get("requested_session_type", "")).strip()
        reception_source = str(r.get("reception_source", "")).strip()
        ...
        if rule is None:
            problems.append("未対応のrequest_type")
            rule = {"needs_source": False, "needs_dest": False, "source_type": "", "dest_type": ""}

        if rule["needs_source"]:
            if pd.isna(pd.to_datetime(src_date, errors="coerce")):
                problems.append("元予定日が不正")
            if not src_slot:
                problems.append("元コマなし")
        if rule["needs_dest"]:
            if pd.isna(pd.to_datetime(dst_date, errors="coerce")):
                problems.append("追加先日が不正")
            if not dst_slot:
                problems.append("追加先コマなし")

        if src_type and rule["source_type"] and src_type != rule["source_type"]:
        ...
        if sid and not problems and rule["needs_source"]:
            ...
            if source_hit.empty:
                ...
                actual_types = set(
                    source_hit["session_type"].fillna("").astype(str).str.strip()
                )
                expected = rule["source_type"]
                if expected == "自習":
                    if actual_types and not actual_types.issubset({"自習"}):
                        problems.append("元予定が自習ではありません")
                elif expected == "授業":
                    if actual_types and actual_types.issubset({"自習"}):
                        problems.append("元予定が授業ではありません")

        if sid and not problems and rule["needs_dest"]:
            same_source_dest = (
                rule["needs_source"]
                and src_date == dst_date
                and src_slot == dst_slot
            )

            # 授業↔自習の同コマ変更は元予定が存在して当然なので重複扱いしない。
            if not (
                same_source_dest
                and req in ["lesson_to_selfstudy", "selfstudy_to_lesson"]
            ):
        ...
        rows.append({
            ...
            "元日": src_date,
            "元コマ": src_slot,
            "元種別": rule.get("source_type", src_type),
            "追加先日": dst_date,
            "追加先コマ": dst_slot,
...
def apply_confirmed_seat_transfers(
    ...
    *,
    ...
    """d380: request_typeに応じて予定変更を一括反映する。"""
    ...
    for _, v in ok.iterrows():
        ...
        req = str(v.get("request_type", "")).strip()
        rule = SEAT_REQUEST_RULES[req]
        src_date = str(v.get("元日", "")).strip()
        src_slot = normalize_slot(v.get("元コマ", ""))
        dst_date = str(v.get("追加先日", "")).strip()
        dst_slot = normalize_slot(v.get("追加先コマ", ""))
        ...
        same_source_dest_conversion = (
            req in ["lesson_to_selfstudy", "selfstudy_to_lesson"]
            and src_date == dst_date
            and src_slot == dst_slot
        )
        ...
        if same_source_dest_conversion:
            ...
            working = upsert_schedule_override_row(
                ...
                slot=dst_slot,
                action="時間変更",
                session_type=rule["dest_type"],
                note=(
                    f"座席予約連携 {rid} / "
                    f"{SEAT_REQUEST_LABELS.get(req, req)} / 種別変更 / "
                    f"受付経路:{reception_source or '口頭受付'}"
                ),
            )
        else:
            if rule["needs_source"] and not _has_exact_cancel(
                working, sid, src_date, src_slot
            ):
                working = upsert_schedule_override_row(
                    working,
                    student_id=sid,
                    ...
                    slot=src_slot,
                    action="キャンセル",
                    session_type=rule["source_type"],
                    note=(
                        f"座席予約連携 {rid} / "
                        f"{SEAT_REQUEST_LABELS.get(req, req)} / 元予定取消 / 取消線表示 / "
                        f"受付経路:{reception_source or '口頭受付'}"
                    ),
                )
            ...
            if rule["needs_dest"]:
                # d382:
                # 座席予約側で一度キャンセルした同じ日時へ再追加する場合、
                # 旧キャンセル行が残っていると build_daily_plan_for_date で
                # 「キャンセルが最後に勝つ」ため新しい追加が見えなくなる。
                # ここでは同一 student/date/slot のうち、
                # 座席予約連携由来のキャンセルだけを解除する。
                if working is not None and not working.empty:
                    _w = working.copy()
                    for _c in ["student_id", "date", "slot", "action", "note"]:
                        if _c not in _w.columns:
                            _w[_c] = ""
                        _w[_c] = _w[_c].fillna("").astype(str).str.strip()

                    _release_cancel_mask = (
                        _w["student_id"].eq(sid)
                        & _w["date"].eq(dst_date)
                        & _w["slot"].map(normalize_slot).eq(dst_slot)
                        & _w["action"].map(normalize_action_value).eq("キャンセル")
                        & _w["note"].str.contains("座席予約連携", na=False)
                    )
                    if _release_cancel_mask.any():
                        working = _w.loc[~_release_cancel_mask].copy()

                note_parts = [
                    f"座席予約連携 {rid}",
                    SEAT_REQUEST_LABELS.get(req, req),
                    "特別追加",
                    f"受付経路:{reception_source or '口頭受付'}",
                ]
                working = upsert_schedule_override_row(
        ...
        applied_rows.append({
            ...
            "source_session_type": rule["source_type"],
            "requested_session_type": rule["dest_type"],
    ...
    if applied_rows:
        log_work = pd.concat(
            [log_work, pd.DataFrame(applied_rows)],
            ignore_index=True,
        )
        log_work = log_work.drop_duplicates(
            subset=["reservation_id"],
            keep="last",
        )
        for c in SEAT_TRANSFER_IMPORT_LOG_COLS:
...
def _new_bot_student_id(used_ids: set[str]) -> str:
    """推測されにくいランダム匿名IDを作る。student_idからは導出しない。"""
    while True:
        candidate = "B" + secrets.token_hex(4).upper()
...
def ensure_bot_student_map_private(students_df: pd.DataFrame) -> pd.DataFrame:
    ...
    used = {x for x in current["bot_student_id"].tolist() if x}
    rows = current.to_dict("records")

    for sid in active_ids:
        if sid in existing_by_sid and existing_by_sid[sid]:
            continue
        bid = _new_bot_student_id(used)
        used.add(bid)
        rows.append({"student_id": sid, "bot_student_id": bid})
...
def build_bot_schedule_export(
    ...
    if pd.isna(start_obj) or pd.isna(end_obj) or start_obj.date() > end_obj.date():
        return pd.DataFrame(columns=BOT_SCHEDULE_COLS)
    ...
    for ts_date in pd.date_range(start_obj.date(), end_obj.date(), freq="D"):
        plan = build_daily_plan_for_date(
            ts_date.date(),
            students_df,
            student_schedule_df,
            monthly_schedule_df,
        ...
        for c in BOT_SCHEDULE_COLS:
            if c not in p.columns:
                p[c] = ""
            p[c] = p[c].fillna("").astype(str).str.strip()
        chunks.append(p[BOT_SCHEDULE_COLS])

    if not chunks:
        return pd.DataFrame(columns=BOT_SCHEDULE_COLS)

    out = pd.concat(chunks, ignore_index=True)
    out["slot"] = out["slot"].map(normalize_slot)
    out = out.drop_duplicates(
        subset=["bot_student_id", "date", "slot"], keep="last"
    )
    out = out.sort_values(
        by=["date", "slot", "bot_student_id"],
        key=lambda col: col.map(
            lambda x: int(x) if col.name == "slot" and str(x).isdigit() else x
        ),
    ).reset_index(drop=True)
    return out[BOT_SCHEDULE_COLS]


def bot_schedule_privacy_check(df: pd.DataFrame) -> tuple[bool, list[str]]:
    """Bot用CSVへ個人情報系の列名が混入していないか確認する。"""
    blocked = {
        "student_id", "display_name", "name", "grade", "phone", "email",
        "address", "note", "memo", "parent", "guardian",
    }
...
def build_daily_plan_for_date(
    ...
    *,
    ...
    if not ov_day.empty:
        ...
        if not add_change_df.empty:
            ...
            if not add_change_df.empty:
                ...
                if not add_view.empty:
                    ...
                    add_view["reason"] = add_view["action_norm"].map(
                        {"時間変更": "時間変更", "追加": "当日追加"}
                    ).fillna("")
                    # d360:
                    # 振替インボックス承認からの追加は、未来日でも
                    # 「当日追加」ではなく「振替追加」と表示する。
                    _inbox_note_s = (
                        add_view["note"]
                        .fillna("")
                        .astype(str)
        ...
        # d383:
        # 同じ student_id × date × slot に座席予約連携の履歴が複数ある場合は、
        # CSV上で最後に登録された受付を最新状態として扱う。
        # 例: 追加 → キャンセル → 再追加 なら、古いキャンセルは無効。
        if not ov_day.empty:
            _seat_hist = ov_day.copy()
            if "note" not in _seat_hist.columns:
                _seat_hist["note"] = ""
            _seat_hist["note"] = _seat_hist["note"].fillna("").astype(str).str.strip()
            _seat_hist["_row_order"] = range(len(_seat_hist))
            _seat_hist["_slot_norm2"] = _seat_hist["slot"].map(normalize_slot)
            _seat_hist["_is_seat_link"] = _seat_hist["note"].str.contains(
                "座席予約連携", na=False
            )

            _seat_only = _seat_hist[_seat_hist["_is_seat_link"]].copy()
            if not _seat_only.empty:
                _latest_map = {}
                for _, _rr in _seat_only.iterrows():
                    _key = (
                        str(_rr.get("student_id", "")).strip(),
                        normalize_slot(_rr.get("slot", "")),
                    )
                    _latest_map[_key] = str(_rr.get("action_norm", "")).strip()

                _stale_cancel_mask = _seat_hist.apply(
                    lambda _rr: (
                        bool(_rr.get("_is_seat_link", False))
                        and str(_rr.get("action_norm", "")).strip() == "キャンセル"
                        and _latest_map.get(
                            (
                                str(_rr.get("student_id", "")).strip(),
                                normalize_slot(_rr.get("slot", "")),
                ...
                if _stale_cancel_mask.any():
                    ov_day = _seat_hist.loc[~_stale_cancel_mask].copy()
                    ov_day = ov_day.drop(
                        columns=[
                            "_row_order",
                            "_slot_norm2",
    ...
    if not plan.empty:
        plan["slot"] = plan["slot"].map(normalize_slot)
        plan = plan.drop_duplicates(subset=["student_id", "date", "slot"], keep="last")
    # d381: 座席予約連携のキャンセルを「完全に消えたように見せず」、
    # 元予定を取消線表示できる行として日次プランへ戻す。
    try:
        _ov_src = schedule_overrides_df.copy() if isinstance(schedule_overrides_df, pd.DataFrame) else pd.DataFrame()
        if not _ov_src.empty:
            for _c in ["student_id", "date", "slot", "action", "session_type", "note"]:
                if _c not in _ov_src.columns:
                    _ov_src[_c] = ""
                _ov_src[_c] = _ov_src[_c].fillna("").astype(str).str.strip()

            _target_str = pd.to_datetime(
                target_date, errors="coerce"
            )
            _target_str = (
                _target_str.strftime("%Y-%m-%d")
                if pd.notna(_target_str)
                else str(target_date)
            )

            _seat_day = _ov_src[
                _ov_src["date"].eq(_target_str)
                & _ov_src["note"].str.contains("座席予約連携", na=False)
            ].copy()

            _cancel_vis = pd.DataFrame(columns=_ov_src.columns)
            if not _seat_day.empty:
                _seat_day["_slot_norm"] = _seat_day["slot"].map(normalize_slot)
                _seat_day["_action_norm"] = _seat_day["action"].map(
                    normalize_action_value
                )
                # 元CSVの行順を保持したまま、同一キーの最後の行だけ採用。
                _seat_day["_order"] = range(len(_seat_day))
                _latest_idx = (
                    _seat_day.groupby(
                        ["student_id", "_slot_norm"],
                        sort=False,
                    )["_order"]
                    .idxmax()
                    .tolist()
                )
                _latest_rows = _seat_day.loc[_latest_idx].copy()
                _cancel_vis = _latest_rows[
                    _latest_rows["_action_norm"].eq("キャンセル")
                ].copy()

            if not _cancel_vis.empty:
                _existing = plan.copy() if isinstance(plan, pd.DataFrame) else pd.DataFrame()
                _rows_to_add = []

                for _, _r in _cancel_vis.iterrows():
                    _sid = str(_r.get("student_id", "")).strip()
                    _slot = normalize_slot(_r.get("slot", ""))
                    _stype = str(_r.get("session_type", "")).strip() or "授業"
                    _note = str(_r.get("note", "")).strip()
                    ...
                    if not _existing.empty:
                        ...
                        for _c in ["student_id", "slot"]:
                            if _c not in _tmp.columns:
                                _tmp[_c] = ""
                            _tmp[_c] = _tmp[_c].fillna("").astype(str).str.strip()
                        _already = bool((
                            _tmp["student_id"].eq(_sid)
                            & _tmp["slot"].map(normalize_slot).eq(_slot)
                        ).any())

                    if not _already:
                        _name = ""
                        try:
                            _sdf = students_df.copy()
                            if "student_id" in _sdf.columns:
                                _hit = _sdf[_sdf["student_id"].fillna("").astype(str).str.strip().eq(_sid)]
                                if not _hit.empty:
                                    for _name_col in ["display_name", "name", "student_name"]:
                                        if _name_col in _hit.columns:
                                            _name = str(_hit.iloc[0].get(_name_col, "")).strip()
                                            if _name:
                                                break
                        except Exception:
                            _name = ""

                        _row = {c: "" for c in _existing.columns} if not _existing.empty else {}

                        # d385:
                        # 取消線表示用の行に、月カレンダー描画で参照される主要列を
                        # 明示的に保持する。ここ以外の予定処理ロジックは変更しない。
                        try:
                            _date_obj = pd.to_datetime(target_date, errors="coerce")
                            _date_str = (
                                _date_obj.strftime("%Y-%m-%d")
                                if pd.notna(_date_obj)
                                else str(target_date)
                            )
                            _weekday_value = (
                                str(int(_date_obj.weekday()))
                                if pd.notna(_date_obj)
                                else ""
                            )
                        except Exception:
                        ...
                        _row.update({
                            ...
                            "display_name": _name,
                            "date": _date_str,
                            "weekday": _weekday_value,
                            "slot": _slot,
                            "session_type": _stype,
                            "status": "キャンセル",
                            "action": "キャンセル",
                            # d386: 月カレンダー既存の通常キャンセル表示へ乗せる。
                        ...
                        # 既存の列名バリエーションがある場合にも値を合わせる。
                        if "name" in _row and not _row.get("name"):
                            _row["name"] = _name
                        if "student_name" in _row and not _row.get("student_name"):
                            _row["student_name"] = _name
                        if "day" in _row and not _row.get("day"):
                            _row["day"] = _date_str

                        _rows_to_add.append(_row)

                if _rows_to_add:
                    _add_df = pd.DataFrame(_rows_to_add)
                    if _existing.empty:
                        plan = _add_df
                    else:
                        for _c in _existing.columns:
                            if _c not in _add_df.columns:
                                _add_df[_c] = ""
                        for _c in _add_df.columns:
                            if _c not in _existing.columns:
                                _existing[_c] = ""
                        plan = pd.concat(
                            [_existing, _add_df[_existing.columns]],
                            ignore_index=True
                        )
    except Exception:
        pass
...
def load_transfer_inbox() -> pd.DataFrame:
    if TRANSFER_INBOX_CSV.exists():
        try:
            df = pd.read_csv(
                TRANSFER_INBOX_CSV,
                dtype=str,
            ...
            df = pd.DataFrame(columns=TRANSFER_INBOX_COLS)
    else:
        df = pd.DataFrame(columns=TRANSFER_INBOX_COLS)
        write_csv_atomic(df, TRANSFER_INBOX_CSV)
        return df

    for c in TRANSFER_INBOX_COLS:
        if c not in df.columns:
            df[c] = ""
        df[c] = df[c].fillna("").astype(str).str.strip()

    if not df.empty:
        _date_raw = df["requested_date"].astype(str).str.strip()
        _date_norm = pd.to_datetime(
            _date_raw,
            errors="coerce",
        ).dt.strftime("%Y-%m-%d")
        df["requested_date"] = _date_norm.fillna(_date_raw)
        df["slot"] = df["slot"].map(normalize_slot)

        _source_date_raw = df["source_date"].astype(str).str.strip()
        _source_date_norm = pd.to_datetime(
            _source_date_raw,
            errors="coerce",
        ).dt.strftime("%Y-%m-%d")
        df["source_date"] = _source_date_norm.fillna(_source_date_raw)
        df["source_slot"] = df["source_slot"].map(normalize_slot)

    # d360: d359までの既存行は振替追加として後方互換。
    df["request_type"] = (
        df["request_type"]
        .fillna("")
        .astype(str)
    ...
    df.loc[df["request_type"].eq(""), "request_type"] = "振替追加"

    return df[TRANSFER_INBOX_COLS].copy()


def save_transfer_inbox(df: pd.DataFrame) -> None:
    ...
    for c in TRANSFER_INBOX_COLS:
        ...
        out[c] = out[c].fillna("").astype(str).str.strip()
    write_csv_atomic(
        out[TRANSFER_INBOX_COLS],
        TRANSFER_INBOX_CSV,
    )


def next_transfer_request_id(df: pd.DataFrame) -> str:
    max_num = 0
    if df is not None and not df.empty:
        for raw in df.get(
            ...
            s = str(raw).strip()
            if s.startswith("TR") and s[2:].isdigit():
                max_num = max(max_num, int(s[2:]))
    return f"TR{max_num + 1:05d}"
...
if page == "閲覧":
    ...
    # d369: 退会済みは通常の対象候補には出さない。
    # 「生徒候補に退会済みも含める」がONの時だけ、末尾に明示して表示する。
    if include_inactive:
    ...
    _sidebar_student_options = ["（全員）"] + student_names
    # d369: 退会済み表示をOFFに戻した時、前回選択していた退会済み生徒を残さない。
    _saved_sidebar_student = str(st.session_state.get("sidebar_student", "（全員）")).strip()
    if _saved_sidebar_student not in _sidebar_student_options:
        st.session_state["sidebar_student"] = "（全員）"

    selected_student = st.sidebar.selectbox(
        "👤 生徒",
        _sidebar_student_options,
        key="sidebar_student",
    )
    ...
    include_inactive = st.sidebar.checkbox(
        ...
        help="通常は在籍中だけを表示します。退会済みの過去記録を確認したい時だけONにしてください。",
    )
    if not include_inactive:
        st.sidebar.caption("退会済みの生徒は対象候補から非表示です。")
  ...
  #  col4.metric("表示中（done）", int(len(filtered_done)))
    ...
    for d in check_days:
        ...
        # 「今日の未完了／昨日以前の未完了」では現在有効な予定だけを対象にする。
        if isinstance(plan_day, pd.DataFrame) and not plan_day.empty:
            _cancel_mask_unfinished = pd.Series(False, index=plan_day.index)

            for _c in ["action", "status", "override_status", "reason"]:
                if _c in plan_day.columns:
                    _vals = plan_day[_c].fillna("").astype(str).str.strip()
                    _cancel_mask_unfinished = _cancel_mask_unfinished | _vals.isin(
                        ["キャンセル", "取消", "取消済み", "キャンセル済み", "削除"]
                    )
            ...
            for _c in ["cancelled", "is_cancelled"]:
                if _c in plan_day.columns:
                    _vals = plan_day[_c]
                    _cancel_mask_unfinished = _cancel_mask_unfinished | _vals.map(
                        lambda v: (
                            v is True
                            or str(v).strip().lower() in ["true", "1", "yes"]
                        )
            ...
            plan_day = plan_day.loc[~_cancel_mask_unfinished].copy()
    ...
    if today_count > 0:
        ...
        if not today_show.empty:
            ...
            _today_base_cols = [c for c in ["生徒", "状態", "コマ", "種別"] if c in today_df.columns]
            ...
            for name, group in today_show.groupby("生徒", dropna=False):
                ...
                _type_values = []
                if "種別" in group.columns:
                    for _v in group["種別"].astype(str).tolist():
                        _v = str(_v).strip()
                        if _v and _v not in _type_values:
                            _type_values.append(_v)
                if "授業" in _type_values and "自習" in _type_values:
                    _type_mark = "📘 授業 / 📝 自習"
                elif "自習" in _type_values:
                    _type_mark = "📝 自習"
                elif "授業" in _type_values:
                    _type_mark = "📘 授業"
                elif _type_values:
                    _type_mark = " / ".join(_type_values)
                else:
                    _type_mark = ""
            ...
            for _, r in today_show.iterrows():
                ...
                type_mark = str(r.get("種別", "")).strip()
                status = str(r.get("状態", "")).strip()
                if type_mark:
                    st.write(f"• {type_mark} / {label} / {status}")
                else:
...
# d316以降は役割が重複している可能性あり。一定期間運用後に削除を検討。
    ...
    # d387:
    # 月カレンダーでは取消線表示のためキャンセル行を残しているが、
    # 「今日の予定」「出席記録」では現在有効な予定だけを対象にする。
    if isinstance(today_view, pd.DataFrame) and not today_view.empty:
        _cancel_mask_today = pd.Series(False, index=today_view.index)

        for _c in ["action", "status", "override_status", "reason"]:
            if _c in today_view.columns:
                _vals = today_view[_c].fillna("").astype(str).str.strip()
                _cancel_mask_today = _cancel_mask_today | _vals.isin(
                    ["キャンセル", "取消", "取消済み", "キャンセル済み", "削除"]
                )
        ...
        for _c in ["cancelled", "is_cancelled"]:
            if _c in today_view.columns:
                _vals = today_view[_c]
                _cancel_mask_today = _cancel_mask_today | _vals.map(
                    lambda v: (
                        v is True
                        or str(v).strip().lower() in ["true", "1", "yes"]
                    )
    ...
    if today_view.empty:
        ...
        if next_candidates.empty:
            ...
            for i, (_, r) in enumerate(next_candidates.iterrows(), start=1):
                ...
                _next_session_type = str(r.get("session_type", r.get("種別", ""))).strip().lower()
                if _next_session_type in ["self", "selfstudy", "自習"] or "自習" in _next_session_type:
                    _next_type_mark = "📝 自習"
                else:
                    _next_type_mark = "📘 授業"
                st.write(
                ...
                def _next_kentei_task_hint(_sid: str, _grade: str) -> str:
                    try:
                        ...
                        _tasks["_order_num"] = pd.to_numeric(
                            _tasks["order"],
                            errors="coerce",
                        ).fillna(9999)
                        _tasks = _tasks.sort_values(
                            ["_order_num", "task_id", "task_name"],
                            na_position="last",
                        ).copy()
        ...
        with st.expander("✅ 出席記録（授業/自習）", expanded=True):
            ...
            if not ids_in_today:
                ...
                if not today_view.empty and {"student_id", "session_type"}.issubset(today_view.columns):
                    ...
                    _count_tv["student_id"] = _count_tv["student_id"].fillna("").astype(str).str.strip()
                    _count_tv["session_type"] = _count_tv["session_type"].fillna("").astype(str).str.strip().str.lower()

                    for _sid_count, _g_count in _count_tv.groupby("student_id"):
                        _lesson_count = int(
                ...
                # d368: 保存済みの出欠回数と、当日スケジュールの予定回数が違うものを常時検出。
                # その場で見落としても、後から修正できる防御層として画面上部に残す。
                ...
                if not target_ids:
                    ...
                    for sid in target_ids:
                        ...
                        with st.container():
                            st.markdown("---")
                            # d371: 準備作業中に見つけやすいよう「準備完了」を最左列へ。
                            c1, c2, c3, c4, c5, c6 = st.columns([1.6, 3, 2, 3, 3, 2])
   ...
   # )
    if sidebar_view_mode == "未来予測（試算）":
        st.subheader("📈 未来予測（試算）")
        st.caption(
            "現在の記録から機械的に試算した目安です。"
            "速さを評価するためではなく、目標設定・教材整理・準備の参考に使います。"
        )

        # =====================================================
        # d334: 全員分の未来予測を先に一覧化
        # =====================================================
        def _build_curriculum_forecast_summary(_sid: str) -> dict:
            _fc_courses = curr_courses.copy()
            _fc_tasks = curr_tasks.copy()
            _fc_prog = curr_prog.copy()

            for _c in [
                "course_id", "course_name", "course_order", "is_active"
            ]:
                if _c not in _fc_courses.columns:
                    _fc_courses[_c] = ""
                _fc_courses[_c] = (
                    _fc_courses[_c].fillna("").astype(str).str.strip()
                )

            for _c in [
                "course_id", "task_id", "task_name", "order",
                "is_active", "student_id",
            ]:
                if _c not in _fc_tasks.columns:
                    _fc_tasks[_c] = ""
                _fc_tasks[_c] = (
                    _fc_tasks[_c].fillna("").astype(str).str.strip()
                )

            for _c in [
                "student_id", "course_id", "task_id",
                "is_done", "is_skip", "done_date",
            ]:
                if _c not in _fc_prog.columns:
                    _fc_prog[_c] = ""
                _fc_prog[_c] = (
                    _fc_prog[_c].fillna("").astype(str).str.strip()
                )

            _fc_courses["_active"] = (
                _fc_courses["is_active"]
                .replace("", "true")
                .str.lower()
                .isin(["true", "1", "yes", "on"])
            )
            _fc_courses["_order"] = pd.to_numeric(
                _fc_courses["course_order"],
                errors="coerce",
            ).fillna(9999)
            ...
            _fc_tasks = _fc_tasks[
                _fc_tasks["_active"]
                & (
                    _fc_tasks["student_id"].eq("")
                    | _fc_tasks["student_id"].eq(_sid)
                )
            ].copy()

            _done_rows = _fc_prog[
                _fc_prog["student_id"].eq(_sid)
                & (
                    _fc_prog["is_done"].str.lower().isin(
                        ["true", "1", "yes", "on", "done", "済", "完了"]
                    )
                    | _fc_prog["is_skip"].str.lower().isin(
                        ["true", "1", "yes", "on", "skip"]
                    )
                )
            ].copy()
            ...
            _done_keys = set(
                zip(
                    _done_rows["course_id"],
                    _done_rows["task_id"],
                )
            )

            # d355:
            # コース単位で完了／スキップ済みのものは未来予測の現在コース候補から除外する。
            _course_closed_ids = set()
            _fc_log = log.copy()
            if not _fc_log.empty:
                for _c in ["student_id", "curriculum", "item", "status", "note"]:
                    if _c not in _fc_log.columns:
                        _fc_log[_c] = ""
                    _fc_log[_c] = _fc_log[_c].fillna("").astype(str).str.strip()

                _closed_rows = _fc_log[
                    _fc_log["student_id"].eq(_sid)
                    & _fc_log["note"].isin(["course_done", "course_skip"])
            ...
            for _, _cr in _fc_courses[
                _fc_courses["_active"]
            ].sort_values("_order").iterrows():
                _cid = str(_cr.get("course_id", "")).strip()
                if _cid in _course_closed_ids:
                    continue
                _ctasks = _fc_tasks[
                    _fc_tasks["course_id"].eq(_cid)
                ].sort_values("_order")
                if _ctasks.empty:
                    continue
                ...
                _done_count = sum(
                    1
                    for _, _tr in _ctasks.iterrows()
                    if (
                        _cid,
                        str(_tr.get("task_id", "")).strip(),
                    ) in _done_keys
                )
                _remaining = max(0, _total - _done_count)
                if _remaining <= 0:
                    continue

                _course_candidates.append(
                    (
                        0 if _done_count > 0 else 1,
                        float(_cr.get("_order", 9999)),
                        _cid,
                        str(_cr.get("course_name", "")).strip() or _cid,
                        _total,
                        _done_count,
                        _remaining,
                    )
            ...
            if not _course_candidates:
                return result

            _course_candidates.sort(key=lambda x: (x[0], x[1]))
            (
                _started_rank,
                _course_order,
                _current_cid,
                ...
                _done_tasks,
                _remaining_tasks,
            ) = _course_candidates[0]

            _pace_rows = _done_rows.copy()
            _pace_rows["_done_dt"] = pd.to_datetime(
                _pace_rows["done_date"],
                errors="coerce",
            )
            _today_ts = pd.Timestamp(date.today())
            _window_start = _today_ts - pd.Timedelta(days=89)
            _recent = _pace_rows[
                _pace_rows["_done_dt"].notna()
                & (_pace_rows["_done_dt"] >= _window_start)
                & (_pace_rows["_done_dt"] <= _today_ts)
            ].copy()
            _recent = _recent.drop_duplicates(
                subset=["course_id", "task_id"],
                keep="last",
            )
            _recent_count = len(_recent)
            _pace_per_month = (
                _recent_count / 90.0 * 30.4375
                if _recent_count > 0
                else 0.0
            )

            _progress_pct = (
                (_done_tasks / _total_tasks * 100.0)
                if _total_tasks > 0
                else 0.0
            )

            result.update({
                "course_id": _current_cid,
                "course_name": _current_cname,
                "done": _done_tasks,
                "total": _total_tasks,
                "remaining": _remaining_tasks,
                "progress_pct": _progress_pct,
                "recent_count": _recent_count,
                "pace_per_month": _pace_per_month,
            })

            if _pace_per_month > 0:
                _months_est = _remaining_tasks / _pace_per_month
                _days_est = max(
                    1,
                    int(round(_months_est * 30.4375)),
                )
                _finish_date = (
                    date.today() + dt.timedelta(days=_days_est)
                )
                _fast_days = max(
                    1,
                    int(round(_days_est * 0.8)),
                )
                _slow_days = max(
                    _fast_days,
                    int(round(_days_est * 1.2)),
                )

                result.update({
                    "months_est": _months_est,
                    "finish_date": _finish_date,
                    "fast_date": (
                        date.today() + dt.timedelta(days=_fast_days)
                    ),
                    "slow_date": (
                        date.today() + dt.timedelta(days=_slow_days)
                    ),
                })

            return result

        def _build_scratch_forecast_summary(_sid: str) -> dict:
            result = {
                "best_grade": None,
                "months_est": None,
                "finish_date": None,
            ...
            _kr = load_kentei_results().copy()
            if _kr.empty:
                return result

            for _c in [
                "student_id", "grade", "result", "pass_date"
            ]:
                if _c not in _kr.columns:
                    _kr[_c] = ""
                _kr[_c] = (
                    _kr[_c].fillna("").astype(str).str.strip()
                )

            _kr.loc[_kr["result"].eq(""), "result"] = "合格"
            _passes = _kr[
                _kr["student_id"].eq(_sid)
                & _kr["result"].isin(["合格", "pass", "passed"])
            ].copy()

            _passes["_grade_num"] = pd.to_numeric(
                _passes["grade"],
                errors="coerce",
            )
            _passes["_date_dt"] = pd.to_datetime(
                _passes["pass_date"],
                errors="coerce",
            )
            _passes = _passes.dropna(subset=["_grade_num"]).copy()

            if _passes.empty:
                result["status"] = "合格履歴なし"
                return result

            _best_grade = int(_passes["_grade_num"].min())
            result["best_grade"] = _best_grade

            if _best_grade <= 1:
                result["status"] = "1級合格済み"
                result["months_est"] = 0.0
                result["finish_date"] = date.today()
                return result

            _dated_passes = _passes.dropna(
                subset=["_date_dt"]
            ).sort_values("_date_dt")

            if len(_dated_passes) < 2:
                result["status"] = "合格履歴不足"
                return result
            ...
            for _a, _b in zip(_rows[:-1], _rows[1:]):
                _grade_step = (
                    float(_a["_grade_num"])
                    - float(_b["_grade_num"])
                )
                _day_step = (
                    _b["_date_dt"] - _a["_date_dt"]
                ).days
                if _grade_step > 0 and _day_step > 0:
                    _intervals.append(
                        _day_step / _grade_step
                    )

            if not _intervals:
                result["status"] = "級アップ履歴不足"
                return result

            _days_per_grade = float(
                pd.Series(_intervals).median()
            )
            _grades_left = _best_grade - 1
            _exam_days = max(
                1,
                int(round(_days_per_grade * _grades_left)),
            )
            _exam_finish = (
                date.today() + dt.timedelta(days=_exam_days)
            )

            result.update({
                "months_est": _exam_days / 30.4375,
                "finish_date": _exam_finish,
                "days_per_grade": _days_per_grade,
                "status": "予測可",
            })
            return result
        ...
        _forecast_students = students.copy()
        if "is_active" in _forecast_students.columns:
            _forecast_students = _forecast_students[
                _forecast_students["is_active"]
                .fillna("")
                .astype(str)
                .str.strip()
                .str.lower()
        ...
        _forecast_rows = []
        for _, _sr in _forecast_students.iterrows():
            _sid = str(_sr.get("student_id", "")).strip()
            _name = str(_sr.get("display_name", "")).strip() or _sid

            _cf = _build_curriculum_forecast_summary(_sid)
            _sf = _build_scratch_forecast_summary(_sid)

            if _cf["months_est"] is None:
                _curr_pred = "データ不足"
            else:
                _curr_pred = f"約{_cf['months_est']:.1f}か月"

            if _cf["finish_date"] is None:
                _curr_finish = "-"
            else:
                _curr_finish = (
                    f"{_cf['finish_date'].year}/"
                    f"{_cf['finish_date'].month}"
                )

            if _sf["best_grade"] is None:
                _scratch_now = "-"
            else:
                _scratch_now = f"{_sf['best_grade']}級"

            if _sf["status"] == "1級合格済み":
                _scratch_pred = "1級合格済み"
            elif _sf["months_est"] is None:
                _scratch_pred = _sf["status"]
            else:
                _scratch_pred = f"約{_sf['months_est']:.1f}か月"

            _forecast_rows.append({
                "生徒": _name,
                "現在コース": _cf["course_name"],
                "進捗": (
                    f"{_cf['done']}/{_cf['total']}"
                    if _cf["total"] > 0
                    else "-"
                ),
                "残り": (
                ...
                "修了予測": _curr_pred,
                "修了目安": _curr_finish,
                "Scratch": _scratch_now,
                "1級予測": _scratch_pred,
            })

        _forecast_table = pd.DataFrame(_forecast_rows)

        if _forecast_table.empty:
            st.info("表示できる在籍生徒がいません。")
        else:
            st.dataframe(
                _forecast_table,
                use_container_width=True,
        ...
        _detail_student_options = (
            _forecast_students["display_name"]
            .fillna("")
            .astype(str)
            .str.strip()
            .replace("", pd.NA)
        ...
        _default_detail_name = (
            selected_student_raw
            if selected_student != "（全員）"
            and selected_student_raw in _detail_student_options
            else (
                _detail_student_options[0]
                if _detail_student_options
                else ""
            )
        )

        if not _detail_student_options:
            st.info("詳細表示できる生徒がいません。")
        else:
            _detail_name = st.selectbox(
                "詳しく見る生徒",
                _detail_student_options,
                index=_detail_student_options.index(
                    _default_detail_name
                ),
                key="future_forecast_detail_student",
            )

            _forecast_student_row = students[
                students["display_name"].astype(str).str.strip()
                == str(_detail_name).strip()
            ].head(1)

            if _forecast_student_row.empty:
                st.warning("生徒情報が見つかりません。")
            else:
                _forecast_sid = str(
                    _forecast_student_row.iloc[0].get(
                        "student_id", ""
                    )
                ).strip()

                st.markdown(
                    f"👤 **{_forecast_sid}｜{_detail_name}**"
                )

                # =================================================
                # 1) 現在のカリキュラム修了予測
                # =================================================
                st.markdown("### 📚 現在のカリキュラム修了まで")

                _cf = _build_curriculum_forecast_summary(
                    _forecast_sid
                )

                if _cf["total"] <= 0:
                    st.success(
                        "現在、予測対象にできる未完了カリキュラムはありません。"
                    )
                else:
                    ...
                    if _cf["months_est"] is None:
                        ...
                        st.markdown(
                            f"#### 目安：**あと約 "
                            f"{_cf['months_est']:.1f}か月**"
                        )
                        st.write(
                            "現在のペースなら、"
                            f"**{_cf['finish_date'].year}年"
                            f"{_cf['finish_date'].month}月ごろ**に"
                            "修了する試算です。"
                        )
                        st.caption(
                            f"予測幅：{_cf['fast_date'].year}年"
                            f"{_cf['fast_date'].month}月 ～ "
                            f"{_cf['slow_date'].year}年"
                            f"{_cf['slow_date'].month}月ごろ"
                        )
                        st.caption(
                            f"計算根拠：直近90日で"
                            f"{_cf['recent_count']}課題完了 "
                            f"→ 月約{_cf['pace_per_month']:.1f}課題。"
                            "欠席・難易度・検定練習などで変動します。"
                        )
                ...
                # =================================================
                st.divider()
                st.markdown("### 🏅 Scratch検定1級合格まで")

                _sf = _build_scratch_forecast_summary(
                    _forecast_sid
                )
                ...
                if _sf["best_grade"] is None:
                    ...
                    if _sf["months_est"] is None:
                        ...
                        st.markdown(
                            f"#### 目安：**あと約 "
                            f"{_sf['months_est']:.1f}か月**"
                        )
                        st.write(
                            "これまでの合格ペースなら、"
                            f"**{_sf['finish_date'].year}年"
                            f"{_sf['finish_date'].month}月ごろ**に"
                            "1級到達する試算です。"
                        )
                        st.caption(
                            f"計算根拠：1級分進むのに"
                            f"中央値 約{_sf['days_per_grade']:.0f}日。"
                            "受験時期や再受験で大きく変動します。"
                        )

        st.divider()
        st.caption(
            "d334試算版：全員一覧→気になる生徒の詳細、"
            "という流れで確認できます。"
        )

    elif sidebar_view_mode == "カリキュラム課題":
        #with tab0:
        #カリキュラム課題
                ...
                if can_render_tab0:
                  ...
                  #  st.markdown(f"👤 **編集対象**：{student_id}｜{selected_student}")
                    ...
                    def _recommend_progress_course_id(_sid: str) -> tuple[str, str]:
                        ...
                        for _cid in _candidate_course_ids:
                            ...
                            if _has_unfinished_task_for_progress(_sid, _cid):
                                ...
                                return _cid, "最新コースが完了・スキップ済み、または未完了課題がないため、次の未完了課題があるコースを選択しています。"

                        # 全部完了/未登録の場合でも、完了・スキップ済みコースは
                        # 「現在のコース」として戻さない。
                        _open_course_ids = [
                            _cid for _cid in _ordered_course_ids
                            if not _course_done_by_log_for_progress(_sid, _cid)
                        ]
                        if (
                            _latest_course_id
                            and _latest_course_id in _open_course_ids
                        ):
                            return _latest_course_id, "未完了課題が見つからないため、未終了の最新コースを表示しています。"
                        if _open_course_ids:
                            return _open_course_ids[0], "未完了課題が見つからないため、未終了コースの先頭を表示しています。"
                        return "", "すべてのコースが完了またはスキップ済みです。"

                    default_course_id, progress_course_reason = _recommend_progress_course_id(student_id)
                    if default_course_id and default_course_id not in course_ids:
                        default_course_id = ""

                    recommended_course_label = (
                        course_label_by_id.get(default_course_id, "")
                    ...
                    selected_course_label = st.selectbox(
                        ...
                        help="完了・スキップ済みコースを避け、次に未完了課題があるコースを推奨表示します。必要なら手動で変更できます。",
                    )
                    selected_course_id = selected_course_label.split("[")[-1].rstrip("]")

                    if not recommended_course_label:
                        st.info(progress_course_reason)
                    elif selected_course_label == recommended_course_label:
                    ...
                    with col_done1:
                        if is_course_done:
                            st.success("✅ 完了済みです")
                            mark_done = False
                            mark_course_skip = False
                        elif is_course_skip:
                            st.info("⏭️ このカリキュラムはスキップ済みです")
                            mark_done = False
                            mark_course_skip = False
                        else:
                            ...
                            mark_course_skip = st.checkbox(
                                "このカリキュラムをスキップする",
                                value=False,
                                key=f"mark_course_skip_{student_id}_{selected_course_id}",
                            )

                    with col_done2:
                        if is_course_done or is_course_skip:
                            if override_done_lock:
                                ...
                                if is_course_skip:
                                    st.warning("編集ロック解除中：カリキュラムのスキップを取り消せます。")
                                    confirm_cancel_course_skip_here = st.checkbox(
                                        "本当にこのカリキュラムのスキップを取り消す",
                                        value=False,
                                        key=f"confirm_cancel_course_skip_here_{student_id}_{selected_course_id}",
                                    )
                                    if st.button(
                                        "↩ カリキュラムのスキップを取り消す",
                                        disabled=not confirm_cancel_course_skip_here,
                                        key=f"cancel_course_skip_here_{student_id}_{selected_course_id}",
                                    ):
                                        df_log = log.copy()
                                        for _c in ["student_id", "curriculum", "item", "status", "note"]:
                                            if _c not in df_log.columns:
                                ...
                                st.caption(
                                    "完了／スキップを取り消す場合は、左の"
                                    "『完了済み課題を編集する』をONにしてください。"
                                )

                        if not is_course_done and not is_course_skip:
                            if mark_done and mark_course_skip:
                                st.warning("完了とスキップはどちらか一方だけ選んでください。")
                            ...
                            if st.button(
                                ...
                                if df_log.empty:
                                    ...
                                    # 同一コースの course_done / course_skip は
                                    # どちらか1件だけになるように置き換える。
                                    for _c in ["student_id", "curriculum", "item", "status", "note"]:
                                        if _c not in df_log.columns:
                                            df_log[_c] = ""
    ...
    if sidebar_view_mode == "検定課題":
        ...
        #   4) 全級合格済みなら1級
            ...
            tasks["order_num"] = pd.to_numeric(
                tasks["order"],
                errors="coerce",
            )
            # d357:
            # 完了/未実施の状態では並べ替えず、
            # kentei_tasks.csv の order を唯一の学習順として使う。
            tasks = tasks.sort_values(
                by=["order_num", "task_id", "task_name"],
                na_position="last",
            )
            ...
            # d357: 検定課題の学習順を編集
            # ---------------------------------------------------------
            # kentei_tasks.csv の order を先生が画面から変更できるようにする。
            # 完了/未実施で並べ替えず、常にこの order 順で表示する。
            # =========================================================
            with st.expander("↕️ 検定課題の順番を編集", expanded=False):
                st.caption(
                    "数字が小さい課題ほど上に表示されます。"
                    "順番を変更して保存すると、課題一覧もその順に並びます。"
                )

                _order_edit_rows = []
                _tasks_for_order = tasks.copy()
                _tasks_for_order["_order_num_edit"] = pd.to_numeric(
                    _tasks_for_order["order"],
                    errors="coerce",
                )

                # order未設定は、現在の表示順の続きとして仮番号を付ける。
                _next_fallback_order = 1
                _used_existing_orders = (
                    _tasks_for_order["_order_num_edit"]
                    .dropna()
                    .astype(int)
                    .tolist()
                )
                if _used_existing_orders:
                    _next_fallback_order = max(_used_existing_orders) + 1

                for _oi, (_, _orow) in enumerate(
                    _tasks_for_order.iterrows(),
                    start=1,
                ):
                    _otid = str(_orow.get("task_id", "")).strip()
                    _otname = str(_orow.get("task_name", "")).strip()
                    _raw_order = pd.to_numeric(
                        pd.Series([_orow.get("order", "")]),
                        errors="coerce",
                    ).iloc[0]
                    if pd.isna(_raw_order):
                        _default_order = _next_fallback_order
                        _next_fallback_order += 1
                    else:
                        _default_order = max(1, int(_raw_order))

                    _new_order = st.number_input(
                        f"{_otname or _otid}",
                        min_value=1,
                        max_value=max(200, len(_tasks_for_order) + 50),
                        value=_default_order,
                        step=1,
                        key=(
                            f"kentei_order_edit_{student_id}_"
                            f"{grade_sel}_{_otid}"
                        ),
                    )
                    _order_edit_rows.append({
                        "task_id": _otid,
                        "task_name": _otname,
                        "order": int(_new_order),
                    })

                if st.button(
                    "💾 この級の課題順を保存",
                    key=f"kentei_order_save_{student_id}_{grade_sel}",
                    use_container_width=True,
                ):
                    _order_df = pd.DataFrame(_order_edit_rows)

                    if _order_df.empty:
                        st.info("保存する課題がありません。")
                    elif _order_df["order"].duplicated().any():
                        _dupes = sorted(
                            _order_df.loc[
                                _order_df["order"].duplicated(keep=False),
                                "order",
                            ]
                            .astype(int)
                            .unique()
                        ...
                        _master = safe_read_csv(
                            KENTEI_TASKS_CSV,
                            ["grade", "task_id", "task_name", "order", "student_id"],
                            stop_on_missing=False,
                        )
                        if _master.empty:
                            st.error("kentei_tasks.csv を読み込めませんでした。")
                        else:
                            for _c in [
                                "grade", "task_id", "task_name",
                                "order", "student_id",
                            ]:
                            ...
                            for _, _erow in _order_df.iterrows():
                                _etid = str(_erow["task_id"]).strip()
                                _new_order_value = str(
                                    int(_erow["order"])
                                )

                                # task_id を主キーとして更新。
                                # gradeも一致する行だけに限定して誤更新を防ぐ。
                                _emask = (
                                    _master["grade"].eq(
                                        str(grade_sel).strip()
                                    )
                                    & _master["task_id"].eq(_etid)
                                )
                                if _emask.any():
                                    _master.loc[
                                        _emask,
    ...
    # 🤖 Bot匿名CSV（d367）
    # ---------------------------
    if admin_section == "🤖 Bot匿名CSV":
        st.subheader("🤖 Bot用匿名スケジュールCSV")
        st.caption(
            "振替Botへ渡す予定データを、実名なしの匿名IDで作成します。"
            "生徒名と匿名IDの対応はこの教室PCだけに保持し、Bot用CSVには出しません。"
        )

        st.info(
            "🔒 サーバーへ送るのは bot_schedule.csv だけ。"
            "bot_student_map_private.csv はローカル専用なのでサーバーへアップロードしません。"
        )

        _bot_col1, _bot_col2 = st.columns(2)
        with _bot_col1:
            _bot_start = st.date_input(
                "出力開始日",
                value=date.today(),
                key="bot_export_start_date",
            )
        with _bot_col2:
            _bot_end = st.date_input(
                "出力終了日",
                value=date.today() + dt.timedelta(days=60),
                key="bot_export_end_date",
            )

        _bot_days = (_bot_end - _bot_start).days + 1
        if _bot_days <= 0:
            ...
            st.caption(
                f"対象期間：{_bot_start.isoformat()} ～ {_bot_end.isoformat()}（{_bot_days}日）"
            )
        ...
        _bot_mapped_ids = set(
            _bot_map_now.get("student_id", pd.Series(dtype=str))
            .fillna("").astype(str).str.strip()
        )
        ...
        if st.button(
            "🔐 匿名IDを準備してBot用CSVを生成",
            type="primary",
            disabled=(_bot_days <= 0 or _bot_days > 120),
            ...
            try:
                ...
                _privacy_ok, _privacy_cols = bot_schedule_privacy_check(_bot_export)
                if not _privacy_ok:
                    st.error(
                        "安全チェックで個人情報系の列を検出したため保存を中止しました："
                        + ", ".join(_privacy_cols)
                    )
                else:
                    write_csv_atomic(_bot_export, BOT_SCHEDULE_CSV)
                    st.session_state["bot_export_ready"] = _bot_export
                    st.success(
                        f"bot_schedule.csv を生成しました（{len(_bot_export)}予定）。"
                    )
            except Exception as e:
                st.error(f"Bot用CSV生成エラー：{e}")

        _bot_export_ready = st.session_state.get("bot_export_ready")
        if _bot_export_ready is None and BOT_SCHEDULE_CSV.exists():
            _bot_export_ready = safe_read_csv(
                BOT_SCHEDULE_CSV,
                BOT_SCHEDULE_COLS,
                stop_on_missing=False,
                show_message=False,
        ...
        if isinstance(_bot_export_ready, pd.DataFrame):
            _privacy_ok, _privacy_cols = bot_schedule_privacy_check(_bot_export_ready)
            if _privacy_ok:
                st.success("✅ 個人情報列チェック：OK（実名・student_id等は含まれていません）")
            else:
                st.error("⚠️ 個人情報列チェック：NG " + ", ".join(_privacy_cols))

            st.markdown("### Botへ渡す内容の確認")
            st.dataframe(
                _bot_export_ready.head(200),
                use_container_width=True,
                hide_index=True,
            )
            if len(_bot_export_ready) > 200:
                st.caption("画面では先頭200件だけ表示しています。CSVには全件保存されています。")

            _bot_csv_bytes = _bot_export_ready.to_csv(
                index=False, encoding="utf-8"
            ).encode("utf-8")
            st.download_button(
                "⬇️ bot_schedule.csv をダウンロード",
                data=_bot_csv_bytes,
                file_name="bot_schedule.csv",
                mime="text/csv",
                key="download_bot_schedule_csv",
            )

        st.markdown("### ローカル専用の匿名ID対応表")
        st.caption(
            "ここは教室PC内だけで確認する情報です。ダウンロードボタンは付けていません。"
            "サーバーへは送らないでください。"
        )
        _bot_map_view = load_bot_student_map_private()
        if _bot_map_view.empty:
            st.info("まだ匿名IDは発行されていません。上の生成ボタンを押すと作成されます。")
        else:
            _bot_names = students[[c for c in ["student_id", "display_name"] if c in students.columns]].copy()
            _bot_map_display = _bot_map_view.merge(_bot_names, on="student_id", how="left")
            _bot_map_display = _bot_map_display[[c for c in ["display_name", "bot_student_id", "student_id"] if c in _bot_map_display.columns]]
            st.dataframe(_bot_map_display, use_container_width=True, hide_index=True)
            st.warning(
    ...
    # 🔁 確定振替CSV取込（d375）
    # ---------------------------
    if admin_section == "🔄 座席予約現在状態CSV":
        st.subheader("🔄 座席予約スケジュールへ返す現在状態CSV")
        st.caption(
            "座席予約スケジュールには受付履歴を残したまま、"
            "カリキュラムツール側で確定している「現在の予定状態」だけを返します。"
        )
        st.info(
            "🔐 出力CSVには氏名・student_idを含めません。"
            " reservation_idを使って座席予約スケジュールv3.7の受付履歴へ現在状態を紐づけます。"
        )
        ...
        if _state_log.empty:
            ...
            _m1, _m2, _m3 = st.columns(3)
            _m1.metric("受付履歴", len(_state_export))
            _m2.metric(
                "現在有効",
                int((_state_export["current_status"] == "active").sum())
                if not _state_export.empty else 0,
            )
            _m3.metric(
                "キャンセル済み",
                int((_state_export["current_status"] == "cancelled").sum())
                if not _state_export.empty else 0,
            )

            if not _state_preview.empty:
                st.markdown("### 教室PC内の確認用")
                st.dataframe(
                    _state_preview,
                    use_container_width=True,
                    hide_index=True,
            ...
            st.download_button(
                "⬇️ 現在状態CSVを出力",
                data=_state_export.to_csv(index=False).encode("utf-8-sig"),
                file_name="reservation_current_state_from_curriculum.csv",
                mime="text/csv",
                use_container_width=True,
                key="download_seat_current_state_sync",
            )
            st.caption(
                "このCSVを座席予約スケジュールv3.7の"
                "「カリキュラムツールから現在状態を同期」へ取り込みます。"
            )

    if admin_section == "🔁 座席予約受付CSV取込":
        st.subheader("🔁 座席予約スケジュール：確定受付CSV取り込み")
        st.caption(
            "座席予約スケジュールv3.5で確定した8種類の受付を取り込みます。"
            "request_typeに応じて、取消・追加・種別変更を自動で分岐します。"
        )
        st.info(
            ...
            if _missing_cols:
                ...
                if _apply_transfer_btn:
                    try:
                        ...
                        st.success(
                            f"✅ {_applied_count}件の受付をrequest_typeに応じて反映しました。"
                        )
                        st.info(
    ...
    # 📥 振替インボックス（d359 MVP）
    # ---------------------------
    if admin_section == "📥 振替インボックス":
        st.subheader("📥 振替インボックス")
        st.caption(
            "保護者連絡をいったん受付箱へ入れ、内容を確認してから"
            "スケジュールへ反映します。"
        )

        inbox_df = load_transfer_inbox()

        # -----------------------------------------------------
        # 表示用：コマ＋時間
        # -----------------------------------------------------
        _transfer_slot_time_map = {}
        if timeslots is not None and not timeslots.empty:
            _ts_for_transfer = timeslots.copy()
            for _c in ["slot", "start", "end"]:
                if _c not in _ts_for_transfer.columns:
                    _ts_for_transfer[_c] = ""
                _ts_for_transfer[_c] = (
                    _ts_for_transfer[_c]
                    .fillna("")
            ...
            # 曜日で同じコマ時間が重複していても、最初の有効値を使う。
            for _, _tr in _ts_for_transfer.iterrows():
                _slot = normalize_slot(_tr.get("slot", ""))
                if not _slot or _slot in _transfer_slot_time_map:
                    continue
                _start = str(_tr.get("start", "")).strip()
                _end = str(_tr.get("end", "")).strip()
                if _start or _end:
                    _transfer_slot_time_map[_slot] = (
                        f"{_slot}｜{_start}〜{_end}"
                    )
                else:
                    _transfer_slot_time_map[_slot] = _slot

        def _transfer_slot_label(_slot):
            _slot_norm = normalize_slot(_slot)
            return _transfer_slot_time_map.get(
                _slot_norm,
                _slot_norm,
            )
        ...
        # 生徒候補
        # -----------------------------------------------------
        _transfer_students = students.copy()
        if "is_active" in _transfer_students.columns:
            _transfer_students = _transfer_students[
                _transfer_students["is_active"]
                .fillna("")
                .astype(str)
                .str.strip()
                .str.lower()
        ...
        for _c in ["student_id", "display_name"]:
            if _c not in _transfer_students.columns:
                _transfer_students[_c] = ""
            _transfer_students[_c] = (
                _transfer_students[_c]
                .fillna("")
                .astype(str)
        ...
        _transfer_students = _transfer_students[
            _transfer_students["student_id"].ne("")
        ].copy()

        _transfer_labels = []
        _transfer_label_to_sid = {}
        for _, _sr in _transfer_students.iterrows():
            _sid = str(_sr.get("student_id", "")).strip()
            _name = str(_sr.get("display_name", "")).strip() or _sid
            _label = f"{_name}｜{_sid}"
            _transfer_labels.append(_label)
            _transfer_label_to_sid[_label] = _sid

        _name_map_transfer = dict(
            zip(
                _transfer_students["student_id"],
                _transfer_students["display_name"],
            )
        )

        _transfer_slot_options = build_slot_options(
            timeslots,
            student_schedule,
            students,
        )
        _transfer_slot_options = [
            normalize_slot(x)
            for x in _transfer_slot_options
            if normalize_slot(x)
            and "その他" not in str(x)
        ]
        _transfer_slot_options = list(
            dict.fromkeys(_transfer_slot_options)
        )

        # =====================================================
        # 1. 受付
        # =====================================================
        st.markdown("### 1．連絡を1件登録")

        if not _transfer_labels:
            st.warning("在籍中の生徒が見つかりません。")
        elif not _transfer_slot_options:
            st.warning(
                "コマ情報が見つかりません。"
                "先にシステム設定でコマ時間を確認してください。"
            )
        else:
            request_type = st.radio(
                ...
                ["振替", "振替追加", "欠席", "自習追加"],
                horizontal=True,
                key="transfer_inbox_request_type",
                help=(
                    "振替＝元予定を取消して振替先を追加。"
                    "振替追加＝振替先だけ追加。"
                    "欠席＝元予定だけ取消。"
                    "自習追加＝元予定を変更せず、自習予定だけ追加。"
                ),
            )

            with st.form(
                "transfer_inbox_add_form",
                clear_on_submit=True,
            ):
                transfer_student_label = st.selectbox(
                    "生徒",
                    _transfer_labels,
                )
                ...
                if request_type in ["振替", "欠席"]:
                    ...
                    source_slot = st.selectbox(
                        "欠席するコマ",
                        _transfer_slot_options,
                        format_func=_transfer_slot_label,
                    )

                transfer_date = None
                transfer_slot = ""
                transfer_session_type = "授業"

                if request_type in ["振替", "振替追加", "自習追加"]:
                    st.markdown(
                        "#### 振替先"
                        if request_type in ["振替", "振替追加"]
                        else "#### 自習予定"
                    )
                    transfer_date = st.date_input(
                        ...
                        value=date.today(),
                        key="transfer_target_date",
                    )
                    transfer_slot = st.selectbox(
                        "振替コマ"
                        if request_type in ["振替", "振替追加"]
                        else "自習コマ",
                        _transfer_slot_options,
                        format_func=_transfer_slot_label,
                        key="transfer_target_slot",
                    )

                    if request_type == "自習追加":
                        transfer_session_type = "自習"
                        st.caption("種別：自習")
                    else:
                        transfer_session_type = st.radio(
                            "種別",
                            ["授業", "自習"],
                            horizontal=True,
                        )
                ...
                transfer_add = st.form_submit_button(
                    "📥 振替インボックスへ登録",
                    type="primary",
                    use_container_width=True,
                )
            ...
            if transfer_add:
                _sid = _transfer_label_to_sid.get(
                    transfer_student_label,
                    "",
                )
                if not _sid:
                    ...
                    _new_request = {
                        ...
                        "slot": (
                            normalize_slot(transfer_slot)
                            if request_type in ["振替", "振替追加", "自習追加"]
                            else ""
                        ),
                        "session_type": (
                            str(transfer_session_type).strip()
                            if request_type in ["振替", "振替追加", "自習追加"]
                            else ""
                        ),
                        "note": str(transfer_note).strip(),
                        "status": "pending",
                        "approved_at": "",
                    }
                    inbox_df = pd.concat(
                        [
                            inbox_df,
                            pd.DataFrame([_new_request]),
                        ],
                        ignore_index=True,
                    )
                    save_transfer_inbox(inbox_df)
                    st.success(
                        f"登録しました："
                        f"{_new_request['request_id']} / "
                        f"{_new_request['request_type']}"
                    )
                    st.rerun()
        ...
        if pending_df.empty:
            st.success("未承認の連絡はありません。")
        else:
            pending_view = pending_df.copy()
            pending_view["生徒"] = pending_view[
                "student_id"
            ].map(_name_map_transfer).fillna(
                pending_view["student_id"]
            )
            pending_view["元コマ"] = pending_view[
                "source_slot"
            ].map(_transfer_slot_label)
            pending_view["振替コマ"] = pending_view[
                "slot"
            ].map(_transfer_slot_label)
            ...
            for _idx, _r in pending_df.iterrows():
                _rid = str(
                    _r.get("request_id", "")
                ).strip()
                _rtype = str(
                ...
                _source_slot = normalize_slot(
                    _r.get("source_slot", "")
                )
                _source_slot_label = _transfer_slot_label(_source_slot)

                _d = str(
                    _r.get("requested_date", "")
                ).strip()
                _slot = normalize_slot(
                    _r.get("slot", "")
                )
                _slot_label = _transfer_slot_label(_slot)
                _stype = str(
                    _r.get("session_type", "")
                ).strip() or "授業"
                _note = str(
                ...
                with st.container(border=True):
                    ...
                    if _rtype == "振替":
                        st.write(
                            f"元：{_source_d} / {_source_slot_label}"
                        )
                        st.write(
                            f"先：{_d} / {_slot_label} / {_stype}"
                        )
                    elif _rtype == "欠席":
                        st.write(
                            f"欠席：{_source_d} / {_source_slot_label}"
                        )
                    elif _rtype == "自習追加":
                        st.write(
                            f"自習追加：{_d} / {_slot_label} / 自習"
                        )
                    else:
                        st.write(
                            f"振替追加：{_d} / {_slot_label} / {_stype}"
                        )
                    if _note:
                        st.caption(f"メモ：{_note}")

                    _source_plan_exists = False
                    if _source_d and _source_slot:
                        _source_date_obj = pd.to_datetime(
                            _source_d,
                            errors="coerce",
                        )
                        if pd.notna(_source_date_obj):
                            _source_plan = build_daily_plan_for_date(
                                _source_date_obj.date(),
                                students,
                                student_schedule,
                                monthly_schedule,
                                schedule_overrides,
                    ...
                    _target_plan_exists = False
                    if _d and _slot:
                        _target_date = pd.to_datetime(
                            _d,
                            errors="coerce",
                        )
                        if pd.notna(_target_date):
                            _target_plan = build_daily_plan_for_date(
                                _target_date.date(),
                                students,
                                student_schedule,
                                monthly_schedule,
                    ...
                    if _rtype == "振替":
                        if not _source_plan_exists:
                            st.warning(
                                "元の予定が見つからないため承認できません。"
                            )
                        if _target_plan_exists:
                            st.warning(
                                "振替先に同じ生徒・日付・コマの予定が"
                                "すでにあるため承認できません。"
                            )
                        approve_disabled = (
                            (not _source_plan_exists)
                            or _target_plan_exists
                        )
                        approve_label = "✅ 承認して振替を反映"

                    elif _rtype in ["振替追加", "自習追加"]:
                        if _target_plan_exists:
                            st.warning(
                                "同じ生徒・日付・コマの予定が"
                                "すでにあります。"
                        ...
                        approve_disabled = _target_plan_exists
                        approve_label = (
                            "✅ 承認して自習を追加"
                            if _rtype == "自習追加"
                            else "✅ 承認して振替を追加"
                        ...
                        approve_disabled = not _source_plan_exists
                        approve_label = "✅ 承認して欠席予定を取消"

                    approve_clicked = st.button(
                        approve_label,
                        key=f"approve_transfer_{_rid}",
                    ...
                    reject_confirm = st.checkbox(
                        "この受付を未承認一覧から削除する（確認）",
                        key=f"reject_transfer_confirm_{_rid}",
                    )
                    reject_clicked = st.button(
                        "🗑 受付を削除（却下扱い）",
                        key=f"reject_transfer_{_rid}",
                        disabled=not reject_confirm,
                        use_container_width=True,
                    )

                    if reject_clicked:
                        inbox_latest = load_transfer_inbox()
                        _mask = (
                            inbox_latest["request_id"]
                            .astype(str)
                            .str.strip()
                            .eq(_rid)
                        )
                        ...
                        save_transfer_inbox(inbox_latest)
                        st.success(
                            "受付を未承認一覧から削除しました。"
                            "履歴には却下済みとして残ります。"
                        )
                    ...
                    if approve_clicked:
                        ...
                        if _rtype == "振替":
                            ...
                            latest_seats = safe_read_csv(
                                SEAT_ASSIGNMENTS_CSV,
                                SEAT_ASSIGNMENT_COLS,
                                stop_on_missing=False,
                            )
                            latest_seats = (
                                remove_seat_assignment_for_plan(
                                    latest_seats,
                                    d=_source_d,
                                    student_id=_sid,
                                    slot=_source_slot,
                            ...
                            approved_overrides = (
                                upsert_schedule_override_row(
                                    ...
                                    slot=_slot,
                                    action="追加",
                                    session_type=_approved_session_type,
                                    note=(
                                        f"振替インボックス承認 "
                                        f"{_rid} / {_approved_note_kind}"
                                        + (
                                            f" / {_note}"
                                            if _note
                                            else ""
                        ...
                        if _rtype == "振替":
                            st.success(
                                "振替を承認しました。"
                                "元予定の取消と振替先の追加を"
                                "schedule_overrides.csv に反映しました。"
                            )
                        elif _rtype == "欠席":
                            st.success(
                                "欠席を承認しました。"
                            ...
                            st.success(
                                "自習追加を承認しました。"
                                "自習予定を schedule_overrides.csv に追加しました。"
                            )
                        else:
                            st.success(
                                "振替追加を承認しました。"
                                "schedule_overrides.csv に追加済みです。"
                            )
                        st.rerun()
        ...
        if not approved_df.empty:
            with st.expander(
                f"✅ 承認済み（{len(approved_df)}件）",
                expanded=False,
            ):
                approved_view = approved_df.copy()
                approved_view["生徒"] = approved_view[
                    "student_id"
                ].map(_name_map_transfer).fillna(
                    approved_view["student_id"]
                )
                approved_view["元コマ"] = approved_view[
                    "source_slot"
                ].map(_transfer_slot_label)
                approved_view["振替コマ"] = approved_view[
                    "slot"
                ].map(_transfer_slot_label)
                approved_view = approved_view.rename(columns={
                    "request_id": "受付ID",
                    "request_type": "内容",
                    "source_date": "元日",
                    "requested_date": "振替日",
        ...
        if not rejected_df.empty:
            with st.expander(
                f"🗑 却下済み（{len(rejected_df)}件）",
                expanded=False,
            ):
                rejected_view = rejected_df.copy()
                rejected_view["生徒"] = rejected_view[
                    "student_id"
                ].map(_name_map_transfer).fillna(
                    rejected_view["student_id"]
                )
                rejected_view["元コマ"] = rejected_view[
                    "source_slot"
                ].map(_transfer_slot_label)
                rejected_view["振替コマ"] = rejected_view[
                    "slot"
                ].map(_transfer_slot_label)
    ...
    # 🎯 検定課題管理（d358）
    # ---------------------------
    if admin_section == "🎯 検定課題管理":
        st.subheader("🎯 検定課題 管理")
        st.caption(
            "検定課題の追加・編集・削除・学習順の変更をここで行います。"
            "閲覧側の「検定課題」は進捗登録用です。"
        )

        _kt_cols = ["grade", "task_id", "task_name", "order", "student_id"]
        _kt = safe_read_csv(
            KENTEI_TASKS_CSV,
            _kt_cols,
            stop_on_missing=False,
        ...
        if _kt.empty:
            _kt = pd.DataFrame(columns=_kt_cols)
        for _c in _kt_cols:
            if _c not in _kt.columns:
                _kt[_c] = ""
            _kt[_c] = _kt[_c].fillna("").astype(str).str.strip()

        _kt["_order_num"] = pd.to_numeric(
            _kt["order"],
            errors="coerce",
        ).fillna(9999)
        _kt = _kt.sort_values(
            ["grade", "_order_num", "task_id", "task_name"],
            na_position="last",
        ).reset_index(drop=True)

        _student_name_map_kt = {}
        try:
            _students_kt = students.copy()
            for _c in ["student_id", "display_name"]:
                if _c not in _students_kt.columns:
                    _students_kt[_c] = ""
                _students_kt[_c] = (
                    _students_kt[_c]
                    .fillna("")
            ...
            _student_name_map_kt = dict(
                zip(
                    _students_kt["student_id"],
                    _students_kt["display_name"],
                )
            )
        except Exception:
            _student_name_map_kt = {}
        ...
        st.markdown("### 現在の検定課題")
        if _kt.empty:
            st.info("検定課題はまだ登録されていません。")
        else:
            _kt_view = _kt.copy()
            _kt_view["対象"] = _kt_view["student_id"].map(
                lambda x: (
                    "共通"
                    if str(x).strip() == ""
                    else _student_name_map_kt.get(
                        str(x).strip(),
                        str(x).strip(),
                    )
                )
            )
            _kt_view = _kt_view.rename(columns={
                "grade": "級",
                "task_id": "task_id",
                "task_name": "課題名",
                "order": "順番",
        ...
        # =====================================================
        with add_col:
            st.markdown("### 🟢 新しい検定課題を追加")

            add_grade = st.selectbox(
                "級",
                ["4", "3", "2", "1"],
                key="kt_admin_add_grade",
            )

            _same_grade_add = _kt[
                _kt["grade"].eq(str(add_grade))
            ].copy()
            _same_grade_orders = pd.to_numeric(
                _same_grade_add["order"],
                errors="coerce",
            ).dropna()
            _next_order = (
                int(_same_grade_orders.max()) + 1
                if not _same_grade_orders.empty
                else 1
            )

            def _suggest_kentei_task_id(_grade: str) -> str:
                _grade = str(_grade).strip()
                _prefix = f"k{_grade}_"
                _nums = []
                for _raw in _kt["task_id"].tolist():
                    _tid = str(_raw).strip()
                    if _tid.startswith(_prefix):
                        _tail = _tid[len(_prefix):]
                        if _tail.isdigit():
                            _nums.append(int(_tail))
                _n = max(_nums) + 1 if _nums else 1
                _candidate = f"{_prefix}{_n:03d}"
                _existing = set(
                    _kt["task_id"].astype(str).str.strip().tolist()
                )
            ...
            add_task_id = st.text_input(
                "task_id（空なら自動提案）",
                value="",
                key="kt_admin_add_task_id",
                placeholder=_suggest_kentei_task_id(add_grade),
            )
            add_task_name = st.text_input(
                "課題名",
                key="kt_admin_add_task_name",
            )
            add_order = st.number_input(
                "順番",
                min_value=1,
                value=int(_next_order),
                step=1,
                key="kt_admin_add_order",
            )

            _student_options_kt = ["共通"]
            _student_label_to_id_kt = {"共通": ""}
            try:
                _stu_for_kt = students.copy()
                if "is_active" in _stu_for_kt.columns:
                    _stu_for_kt = _stu_for_kt[
                        _stu_for_kt["is_active"]
                        .fillna("")
                        .astype(str)
                        .str.strip()
                        .str.lower()
                ...
                for _, _sr in _stu_for_kt.iterrows():
                    _sid = str(_sr.get("student_id", "")).strip()
                    _sname = str(
                        _sr.get("display_name", "")
                    ).strip()
                    if not _sid:
                        continue
                    _label = f"{_sname or _sid}｜{_sid}"
                    _student_options_kt.append(_label)
                    _student_label_to_id_kt[_label] = _sid
            except Exception:
                pass

            add_student_label = st.selectbox(
                "対象",
                _student_options_kt,
                key="kt_admin_add_student",
                help="通常は「共通」。特定の生徒だけに出す場合のみ個別指定します。",
            )

            if st.button(
                "🟢 検定課題を追加",
                key="kt_admin_add_btn",
                use_container_width=True,
            ):
                _grade = str(add_grade).strip()
                _tid = (
                    str(add_task_id).strip()
                    or _suggest_kentei_task_id(_grade)
                )
                _tname = str(add_task_name).strip()
                _sid = _student_label_to_id_kt.get(
                    add_student_label,
                    "",
                )
                ...
                if not _tname:
                    ...
                    _new = pd.DataFrame([{
                        ...
                        "task_id": _tid,
                        "task_name": _tname,
                        "order": str(int(add_order)),
                        "student_id": str(_sid).strip(),
                    }])
                    _save = pd.concat(
                        [_kt[_kt_cols], _new],
                        ignore_index=True,
        ...
        with edit_col:
            ...
            if _kt.empty:
                st.info("編集できる検定課題がありません。")
            else:
                _edit_options = _kt.index.tolist()

                def _kt_edit_label(_idx):
                    _r = _kt.loc[_idx]
                    _target = (
                        "共通"
                        if str(_r.get("student_id", "")).strip() == ""
                    ...
                    return (
                        f"{_r.get('grade','')}級｜"
                        f"{_r.get('order','')}｜"
                        f"{_r.get('task_name','')}｜{_target}"
                    )

                edit_idx = st.selectbox(
                    "編集する課題",
                    _edit_options,
                    format_func=_kt_edit_label,
                    key="kt_admin_edit_target",
                )
                _er = _kt.loc[edit_idx]

                edit_grade = st.selectbox(
                    "級（編集）",
                    ["4", "3", "2", "1"],
                    index=["4", "3", "2", "1"].index(
                        str(_er.get("grade", "")).strip()
                    )
                    if str(_er.get("grade", "")).strip()
                    in ["4", "3", "2", "1"]
                    else 0,
                    key=f"kt_admin_edit_grade_{edit_idx}",
                )
                st.text_input(
                    "task_id（変更不可）",
                    value=str(_er.get("task_id", "")),
                    disabled=True,
                    key=f"kt_admin_edit_tid_{edit_idx}",
                )
                edit_name = st.text_input(
                    "課題名（編集）",
                    value=str(_er.get("task_name", "")),
                    key=f"kt_admin_edit_name_{edit_idx}",
                )
                try:
                    _edit_order_default = int(
                        float(_er.get("order", 1) or 1)
                    )
                except Exception:
                    _edit_order_default = 1
                edit_order = st.number_input(
                    "順番（編集）",
                    min_value=1,
                    value=max(1, _edit_order_default),
                    step=1,
                    key=f"kt_admin_edit_order_{edit_idx}",
                )
                ...
                _current_label = "共通"
                for _label, _sid in _student_label_to_id_kt.items():
                    if _sid == _current_sid:
                        _current_label = _label
                        break

                edit_student_label = st.selectbox(
                    "対象（編集）",
                    _student_options_kt,
                    index=_student_options_kt.index(_current_label),
                    key=f"kt_admin_edit_student_{edit_idx}",
                )

                if st.button(
                    "🟠 編集内容を保存",
                    key=f"kt_admin_edit_save_{edit_idx}",
                    use_container_width=True,
                ):
                    _new_grade = str(edit_grade).strip()
                    _tid = str(_er.get("task_id", "")).strip()
                    _new_name = str(edit_name).strip()
                    _new_sid = _student_label_to_id_kt.get(
                        edit_student_label,
                        "",
                    )

                    _dup_mask = (
                        _kt["grade"].eq(_new_grade)
                        & _kt["task_id"].eq(_tid)
                        & (_kt.index != edit_idx)
                    )
                    if not _new_name:
                        ...
                        st.error(
                            "変更先の級に同じtask_idがあります。"
                        )
                    else:
                        _save = _kt[_kt_cols].copy()
                        _save.loc[edit_idx, "grade"] = _new_grade
                        _save.loc[edit_idx, "task_name"] = _new_name
                        _save.loc[edit_idx, "order"] = str(
                            int(edit_order)
                        )
                        _save.loc[edit_idx, "student_id"] = str(
                            _new_sid
                        ).strip()
                        ...
                        try:
                            ...
                            pass
                        st.success("検定課題を更新しました。")
                        st.rerun()

                st.markdown("#### 🗑 削除")
                _delete_progress_exists = False
                try:
                    _delete_progress_exists = bool(
                        (
                            kentei_prog["grade"]
                            .astype(str)
                            .str.strip()
                            .eq(str(_er.get("grade", "")).strip())
                            & kentei_prog["task_id"]
                            .astype(str)
                            .str.strip()
                            .eq(str(_er.get("task_id", "")).strip())
                        ).any()
                    )
                except Exception:
                    pass

                if _delete_progress_exists:
                    st.warning(
                        "この課題には生徒の進捗記録があります。"
                        "削除すると課題マスタだけ消え、進捗履歴は残ります。"
                    )

                delete_confirm = st.checkbox(
                    "この検定課題を削除する（確認）",
                    key=f"kt_admin_delete_confirm_{edit_idx}",
                )
                if st.button(
                    "🗑 検定課題を削除",
                    disabled=not delete_confirm,
                    key=f"kt_admin_delete_btn_{edit_idx}",
                    use_container_width=True,
                ):
                    _save = _kt.drop(
                        index=edit_idx,
                        errors="ignore",
                    )[_kt_cols].reset_index(drop=True)
                    write_csv_atomic(
                        _save,
                        KENTEI_TASKS_CSV,
                    )
                    try:
                        ...
                        pass
                    st.success("検定課題を削除しました。")
                    st.rerun()

        st.divider()
        st.caption(
            "順番はこの画面の「順番」で変更できます。"
            "閲覧 → 検定課題側の順番編集と同じ kentei_tasks.csv を更新します。"
        )
    ...
    if admin_section == "🗓️ 月スケジュール":
        ...
        elif monthly_view_mode == "🪑 今日の座席":
            st.info(
                "月スケジュールと同じ画面で、今日の座席配置を確認・編集します。"
            )
            render_integrated_seat_view(monthly_schedule)
            st.stop()
        else:
        ...
        if hasattr(st, "dialog"):
            ...
            def _open_monthly_edit_dialog():
                ...
                if selected_source == "override":
                    ...
                    if is_seat_linked_transfer:
                        _external_source_icon = (
                            "📱" if "受付経路:LINE" in selected_note
                            or "LINE Bot" in selected_note
                            or "LINEBot" in selected_note
                            else "😃"
                        )
                        st.info(
                            f"{_external_source_icon} 外部受付から反映された予定です。"
                            " 現在は連携履歴を守るため、このツールでは内容変更・誤登録訂正は行いません。"
                            " 変更や入力ミスの訂正は座席予約スケジュール側で行ってください。"
                            " この画面では内容確認とキャンセルだけできます。"
                        )
                        if linked_source.get("found"):
                            st.caption(
                                f"元予定：{linked_source.get('source_date', '')} / "
                                f"{format_slot_label(linked_source.get('source_slot', ''), monthly_slot_label_map)}"
                            )
                    ...
                    if is_seat_linked_transfer:
                        ...
                        st.text_input(
                            "日付",
                            value=selected_date_s,
                            disabled=True,
                            key=f"external_readonly_date_{target_year}_{target_month}_{selected_override_idx}",
                        )
                        st.text_input(
                            "コマ",
                            value=format_slot_label(selected_slot, monthly_slot_label_map),
                            disabled=True,
                            key=f"external_readonly_slot_{target_year}_{target_month}_{selected_override_idx}",
                        )
                        st.text_input(
                            "種別",
                            value=selected_type,
                            disabled=True,
                            key=f"external_readonly_type_{target_year}_{target_month}_{selected_override_idx}",
                        )
                        st.text_input(
                            "連携メモ（自動管理）",
                            value=selected_note,
                            disabled=True,
                            key=f"external_readonly_note_{target_year}_{target_month}_{selected_override_idx}",
                        )
                    else:
                        override_dialog_key = (
                    ...
                    st.divider()
                    st.markdown("### 予定をキャンセル")
                    if is_seat_linked_transfer:
                        st.caption(
                            "この外部受付由来の現在予定をキャンセルします。"
                            " 入力ミスの訂正や日時・種別の変更は受付元で行います。"
                        )
                    if st.button(
                        "↩ 予定をキャンセル",
                        key=(
                    ...
                    if not is_seat_linked_transfer:
                        st.divider()
                        st.caption(
                            "通常の例外追加で入力を間違えた場合だけ「誤登録を訂正」できます。"
                            " 訂正するとカレンダーから消えますが、訂正履歴は残ります。"
                        )
                        confirm_override_delete = st.checkbox(
                            "これはキャンセルではなく、入力ミスの訂正です",
                            key=(
                        ...
                        if st.button(
                            ...
                            _correction_row = (
                                schedule_overrides.loc[selected_override_idx]
                                if selected_override_idx in schedule_overrides.index
                                else {}
                            )
                            append_schedule_correction_log({
                                "source": "💻 ツール・例外予定",
                                "student_id": selected_sid,
                                "date": selected_date_value,
                                "slot": selected_slot,
                                "session_type": (
                                    str(_correction_row.get("session_type", "")).strip()
                                    if hasattr(_correction_row, "get") else ""
                                ),
                                "reason": "例外予定の誤登録",
                                "note": (
                                    str(_correction_row.get("note", "")).strip()
                                    if hasattr(_correction_row, "get") else ""
                                ),
                            })
                ...
                st.info(
                    "予定をキャンセル＝実際にあった予定を取りやめ、取消線を残します。"
                    "誤登録を訂正＝入力ミスだった予定をカレンダーから消しますが、訂正履歴は残します。"
                )
        ...
        if monthly_view_mode == "📅 編集":
            ...
            if hasattr(st, "dialog"):
                ...
                def _open_monthly_add_dialog():
                    ...
                    # d369: 種別はフォーム外に置き、変更時に回数区分候補を即時連動させる。
                    add_type = st.radio(
                        "追加する種別",
                        ["授業", "自習"],
                        horizontal=True,
                        key=f"monthly_add_type_{target_year}_{target_month}",
                    )
                    add_type_norm = str(add_type).strip()

                    if add_type_norm == "自習":
                        _add_reason_options = ["自習"]
                        st.caption("自習は回数外として登録します。回数区分は「自習」に固定されます。")
                    else:
                        _add_reason_options = [
                            r for r in monthly_reason_options
                            if str(r).strip() != "自習"
                    ...
                    with st.form(
                        ...
                        add_reason = st.selectbox(
                            ...
                            _add_reason_options,
                            index=(
                                _add_reason_options.index("通常")
                                if "通常" in _add_reason_options
                                else 0
                            ),
                            format_func=format_monthly_reason_label,
                            disabled=(add_type_norm == "自習"),
                        )
                    ...
                    if add_submit:
                        if not add_sid:
                            ...
                            effective_reason = str(add_reason).strip()
                            add_type_norm = str(add_type).strip()

                            # d369: 保存時も二重チェック。
                            # 自習は必ず回数外の「自習」、授業に「自習」区分は保存しない。
                            if add_type_norm == "自習":
                                effective_reason = "自習"
                            elif add_type_norm == "授業" and effective_reason in ["", "自習"]:
                                effective_reason = "通常"
            ...
            with monthly_cal_col:
                ...
                st.caption("予定の確認・変更・キャンセル・誤登録の訂正は、基本的にこのカレンダーから行います。")
                st.info("操作の意味：予定をキャンセル＝実際にあった予定を取りやめ、取消線を残します / 誤登録を訂正＝入力ミスの予定をカレンダーから消しますが訂正履歴は残します / 解除＝キャンセルを取り消して予定を復活させます / 変更・振替＝日時や種別を変更します。")
                ...
                for _day_ts in pd.date_range(month_start, month_end, freq="D"):
                    ...
                    _plan.loc[_override_like & _reason_s.str.contains("振替追加|当日追加|追加", regex=True, na=False), "override_status"] = "追加"
        ...
        if monthly_view_mode in ("📅 編集", "⚙ 生成"):
            ...
            # 未保存変更がある場合は、同じ固定バーに保存・破棄も並べる。
            if monthly_view_mode == "📅 編集" or monthly_has_unsaved:
                st.markdown(
                    """
                    <style>
            ...
            if save_monthly_batch:
                ...
                if current_signature != base_signature:
                    ...
                    if pending_cleanup:
                        ...
                        _correction_items = [
                            item for item in pending_cleanup
                            if str(item.get("correction_operation", "")).strip()
                            == "誤登録を訂正"
                        ]
                        if _correction_items:
                            append_schedule_correction_log(_correction_items)
